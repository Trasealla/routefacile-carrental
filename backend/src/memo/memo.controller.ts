...
/**
 * End-user memo APIs. Any authenticated user (admins.* row) can hit these.
 * Visibility is enforced by MemoAccessResolverService.
 * Mounted at /api/v1/memo (no /admin prefix).
 */
@ApiExcludeController()
@UseGuards(AdminJwtAuthGuard)
@Controller('memo')
export class MemoEndUserController {
  constructor(
  ...
  async categories() {
    return await this.categoryService.getAll(
      { status: 1 }, ['id', 'name', 'slug'], {}, null, false, 1, 1000,
  ...
  async listing(
    ...
    return await this.resolver.listVisibleDocuments(req.user.id, req.user.type, params);
  }

  @Get('documents/:id')
  async detail(@Param('id') id: number, @Request() req) {
    const r = await this.resolver.resolveViewable(req.user.id, req.user.type, Number(id));
  ...
  /**
   * Lightweight view-event endpoint. The frontend can call this once when
   * the detail page mounts so the counter bumps even if the user does not
   * download/stream the file. Same dedupe rules as the file-fetch route.
   */
  ...
  async viewEvent(@Param('id') id: number, @Request() req) {
    ...
    if (counted) {
      await this.auditService.record(req.user.id, 'document.view', 'memo_documents', r.document.id, { version_id: r.version.id, source: 'view-event' }, req.ip);
    }
  ...
  /**
   * Stream the current version of the memo for in-browser inline viewing.
   * Content-Disposition: inline; no caching; audit logged.
   */
  ...
  async view(@Param('id') id: number, @Request() req, @Res() res: Response) {
    ...
    await this.auditService.record(req.user.id, 'document.view', 'memo_documents', r.document.id, { version_id: r.version.id }, req.ip);
