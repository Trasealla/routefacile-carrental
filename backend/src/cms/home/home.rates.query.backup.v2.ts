...
export function homeRateQueryBackupV2(
    city_id: number,
    from_date: string,
    ...
    FROM
        ...
        LEFT JOIN rates_range AS rr ON (rr.id = (SELECT MAX(rr_inner.id) from rates_range rr_inner 
                WHERE c.group_id = rr_inner.group_id
                AND rr_inner.city_id = ${city_id}
                AND rr_inner.location_id IS NULL
                AND rr_inner.deleted_at IS NULL
                AND rr_inner.end_date >= Now()
                AND rd.date BETWEEN rr_inner.start_date AND rr_inner.end_date
                AND 1 BETWEEN rr_inner.from AND rr_inner.to))
        WHERE
        ...
        LEFT JOIN rates_range AS rr ON (rr.id = (SELECT MAX(rr_inner.id) 
                FROM rates_range rr_inner 
                WHERE c.group_id = rr_inner.group_id
                AND rr_inner.city_id = ${city_id}
                AND rr_inner.location_id IS NULL
                AND rr_inner.deleted_at IS NULL
                AND rr_inner.end_date >= Now()
                AND rd.date BETWEEN rr_inner.start_date AND rr_inner.end_date
                AND 7 BETWEEN rr_inner.from AND rr_inner.to))
        WHERE
        ...
        FROM
            cars AS c
        LEFT JOIN car_categories AS cc on cc.id = c.category_id 
        LEFT JOIN rates_monthly_v2 AS rm ON (rm.car_id = c.id AND (rm.model_year IS NULL OR rm.model_year = c.year))    
        WHERE
            mileage = 3000 
            AND months = 1 
            AND city_id = ${city_id} 
