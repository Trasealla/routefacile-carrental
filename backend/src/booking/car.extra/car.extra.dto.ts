...
import { IsNotEmpty, IsString, IsEnum, ValidateIf, IsNumber, IsDateString, Validate, Min, Max, Matches, IsOptional, IsArray, ArrayNotEmpty, ArrayMaxSize } from "class-validator";
...
export class CarExtraDto {
    ...
    @ValidateIf(o => o.booking_type === BookingTypes.DAILY)
    ...
    @ApiProperty({ example: 'monthly_mileage required for booking_type monthly' })
    @ValidateIf(o => o.booking_type === BookingTypes.MONTHLY)
    @IsNumber()
    @Type(() => Number)
    @IsNotEmpty()
