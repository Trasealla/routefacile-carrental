...
import { EditBookingEvent } from '../events/edit.booking.event';
import { EDIT_BOOKING } from 'src/mail/sms/sms_texts';

@Injectable()
export class EditBookingListener {
    constructor(
    ...
    @OnEvent('edit.booking')
    async handleEditBookingEvent(event: EditBookingEvent) {
        try {
            ...
            const relations = {
                ...
                monthly_installments: {
                    columns: ['id', 'installment_no', 'due_date', 'sub_amount', 'vat_amount', 'total_amount', 'refund_amount']
                }
            }
            const booking = await this.bookingRepoService.getOne(where, [], relations, BookingRepoService.LEFT_JOIN);
            
            if (!booking) {
                console.error(`[EditBookingListener] Booking not found for id: ${event.booking_id}`);
                return;
            }
            ...
            const sms_text = EDIT_BOOKING.replace('[user]', booking.user_first_name).replace('[booking_number]', booking.booking_number);
            ...
            try {
                ...
                console.error(`[EditBookingListener] SMS failed for booking ${booking.id}:`, smsError);
            }
            ...
            try {
                ...
                console.log(`[EditBookingListener] Email sent successfully for booking ${booking.id} to ${booking.user_email}`);
            } catch (emailError) {
                console.error(`[EditBookingListener] Email failed for booking ${booking.id}:`, emailError);
            }
        } catch (error) {
            console.error(`[EditBookingListener] Error processing edit booking for id ${event.booking_id}:`, error);
        }
    }
}
