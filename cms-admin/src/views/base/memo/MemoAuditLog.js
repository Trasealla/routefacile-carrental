  ...
  return (
    <div className="memo-page">
      <div className="memo-page__header">
        <div>
          <h1 className="memo-page__title">{t("Audit Log")}</h1>
          <p className="memo-page__subtitle">
            {t(filtered.length === 1
              ? "Every action across the Memo Portal — {{count}} event."
              : "Every action across the Memo Portal — {{count}} events.", { count: filtered.length })}
          </p>
        </div>
        <button className="memo-pillbtn memo-pillbtn--primary" onClick={exportCsv}>
          <FaDownload size={11} /> {t("Export CSV")}
        </button>
      </div>
      ...
      <div className="memo-filterbar">
        <div className="memo-filterbar__search">
          ...
          <input
            placeholder={t("Search actor or entity...")}
            value={q}
      ...
      {dateRangeInvalid && (
        <div className="memo-card" style={{ borderLeft: "3px solid var(--memo-danger)", color: "var(--memo-danger)" }}>
          {t('"From" date is after "To" date — please fix the range.')}
        </div>
      )}
      ...
      {loading ? (
        <div className="memo-card text-center py-5">
          <Loader color="indigo" />
          <p className="text-muted small mt-2 mb-0">{t("Loading activity…")}</p>
        </div>
      ) : error ? (
        <div className="memo-card memo-empty">
          ...
          <h5>{t("Could not load activity")}</h5>
        ...
        <div className="memo-card memo-empty">
          ...
          <h5>{t("No activity matches your filters")}</h5>
          <p className="mb-0">{t("Try widening your date range.")}</p>
        </div>
      ) : (
              ...
              <div className="memo-card">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <h3 className="memo-card__title mb-0">
                    {day === "unknown" ? t("Undated") : formatDate(day)}
                  </h3>
                  <span className="text-muted small">{t("{{count}} events", { count: events.length })}</span>
                </div>
                      ...
                      >
                        ...
                        <div className="memo-timeline__body">
                          <p className="memo-timeline__line">
                            <strong>{a.actor || t("System")}</strong> {meta.label}{" "}
