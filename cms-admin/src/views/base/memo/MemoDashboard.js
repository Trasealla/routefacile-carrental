  ...
  return (
    <div className="memo-page">
      <div className="memo-page__header">
        <div>
          <h1 className="memo-page__title">{t("Memo Portal Dashboard")}</h1>
          <p className="memo-page__subtitle">
            {t("Today is {{date}}. Here's how things are going.", { date: formatDate(new Date().toISOString()) })}
          </p>
        </div>
        <button onClick={createH.open} className="memo-pillbtn memo-pillbtn--primary">
          <FaPlus size={11} /> {t("New Memo")}
        </button>
      </div>
      ...
      {loading && (
        <div className="memo-card text-center py-5">
          <Loader color="indigo" />
          <p className="text-muted small mt-2 mb-0">{t("Loading dashboard…")}</p>
        </div>
      )}
      ...
      {!loading && error && (
        <div className="memo-card memo-empty">
          ...
          <h5>{t("Could not load dashboard")}</h5>
      ...
      <div className="row g-3 mb-4">
        <div className="col-12 col-sm-6 col-xl-3">
          <KpiCard
            ...
            value={stats.total}
            hint={t("{{published}} published · {{draft}} draft · {{archived}} archived", {
        ...
        <div className="col-12 col-sm-6 col-xl-3">
          <KpiCard
            index={1}
            variant="2"
            label={t("Total Admin Users")}
            value={stats.adminUsers ?? "—"}
            hint={stats.adminRoles ? t("{{roles}} roles across the org", { roles: stats.adminRoles }) : t("Awaiting backend stats")}
            icon={<FaUsers />}
          />
        </div>
        <div className="col-12 col-sm-6 col-xl-3">
        ...
        <div className="col-12 col-sm-6 col-xl-3">
          <KpiCard
            ...
            hint={stats.activeUsersPct != null ? t("{{pct}}% of admin users", { pct: stats.activeUsersPct }) : "—"}
            icon={<FaUserCheck />}
          />
        </div>
      </div>
      ...
      <div className="row g-3">
        ...
        <div className="col-12 col-xl-5">
          ...
          >
            <div className="d-flex justify-content-between align-items-center mb-3">
              <div>
                <h3 className="memo-card__title">{t("Recently Published")}</h3>
                <p className="memo-card__subtitle mb-0">{t("Last 5 memos pushed live")}</p>
              </div>
            ...
            <ul className="list-unstyled m-0">
              {stats.recentlyPublished.length === 0 && (
                <li className="text-muted small py-3">{t("No published memos yet.")}</li>
              )}
        ...
        <div className="col-12 col-xl-4">
          ...
          >
            <h3 className="memo-card__title">{t("Top Viewed")}</h3>
            <p className="memo-card__subtitle">{t("Most opened memos this month")}</p>
            {stats.topViewed.length === 0 && (
              <p className="text-muted small mb-0">{t("No view data yet.")}</p>
            )}
        ...
        <div className="col-12 col-xl-3">
          ...
          >
            <div className="d-flex justify-content-between align-items-center mb-3">
              <div>
                <h3 className="memo-card__title">{t("Recent Activity")}</h3>
                <p className="memo-card__subtitle mb-0">{t("Live feed")}</p>
              </div>
            ...
            <ul className="memo-timeline">
              {recentActivity.length === 0 && (
                <li className="text-muted small">{t("No recent activity.")}</li>
              )}
