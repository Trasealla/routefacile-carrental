...
const Phase2Pill = () => {
  ...
  return (
    <span
      ...
      title={t("Backend support required")}
    >
      {t("Phase 2")}
    </span>
  );
...
const MemoSettings = () => {
  ...
  const save = () => notifySuccess(t("Settings saved"));

  return (
    <div className="memo-page">
      <div className="memo-page__header">
        <div>
          <h1 className="memo-page__title">{t("Memo Portal Settings")}</h1>
          <p className="memo-page__subtitle">
            {t("Tune how memos are uploaded, watermarked and delivered to end users.")}
          </p>
        </div>
        <button className="memo-pillbtn memo-pillbtn--primary" onClick={save}>
          <FaSave size={11} /> {t("Save Changes")}
        </button>
      </div>
      ...
      <Section
        ...
        description={t("Read-only. Editing requires backend support.")}
      >
        ...
        <div className="d-flex align-items-center gap-2 text-muted small">
          <FaInfoCircle /> {t("Maximum upload size:")} <strong className="ms-1 text-dark">25 MB</strong>
      ...
      <Section
        index={1}
        title={t("PDF Watermark")}
        description={t("Overlay shown diagonally across the end-user PDF viewer.")}
      >
        <label className="form-label small text-muted">{t("Template")}</label>
        <input
        ...
        <div className="text-muted small mt-2">
          {t("Available variables:")} <code>{"{user_email}"}</code>, <code>{"{datetime}"}</code>, <code>{"{ip}"}</code>
        </div>
        ...
        >
          ...
          <small className="text-muted d-block mt-2">{t("Live preview")}</small>
        </div>
      </Section>
      ...
      <Section
        ...
        description={t("How long a portal user stays signed in before re-authenticating.")}
      >
        ...
        <small className="text-muted">{t("Recommended: 8 hours.")}</small>
      </Section>

      <Section
        index={3}
        title={t("Email Templates")}
        description={t("Used for the sign-in PIN and the welcome email.")}
      >
        <label className="form-label small text-muted">{t("Subject")}</label>
        <input
          className="form-control mb-3"
          value={emailSubject}
          onChange={(e) => setEmailSubject(e.target.value)}
        />
        <label className="form-label small text-muted">{t("Body")}</label>
        <textarea
          className="form-control"
          rows={6}
          value={emailBody}
        ...
        />
        <div className="text-muted small mt-2">
          {t("Variables:")} <code>{"{pin}"}</code>, <code>{"{user_email}"}</code>, <code>{"{expires_in}"}</code>
        </div>
      </Section>
    </div>
  );
