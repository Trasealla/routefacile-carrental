...
const MemoCategoriesList = () => {
  ...
  const validate = () => {
    ...
    if (!name) e.name = t("Name is required");
    if (!slug) e.slug = t("Slug is required");
    else if (!/^[a-z0-9-]+$/.test(slug))
      e.slug = t("Lowercase letters, numbers and hyphens only");
    else if ((rows || []).some((r) => r.slug === slug && r.id !== form.id))
      e.slug = t("Slug already exists");
    setErrors(e);
  ...
  const submit = async () => {
    ...
    try {
      if (form.id) {
        ...
        notifySuccess(t("Category updated"));
      } else {
        await memoApi.createCategory(clean);
        notifySuccess(t("Category created"));
      }
      await reload();
      close();
  ...
  const remove = async (row) => {
    if ((row.docs || row.documentsCount || 0) > 0) {
      notifyError(t("Cannot delete: this category is in use."));
      return;
    }
    if (!window.confirm(t('Delete category "{{name}}"?', { name: row.name }))) return;
    try {
  ...
  return (
    <div className="memo-page">
      <div className="memo-page__header">
        <div>
          <h1 className="memo-page__title">{t("Categories")}</h1>
          <p className="memo-page__subtitle">
            {t("Group memos for easier discovery. {{count}} of {{total}} shown.", {
      ...
      >
        {loading ? (
          <div className="text-center py-5">
            <Loader color="indigo" />
            <p className="text-muted small mt-2 mb-0">{t("Loading categories…")}</p>
          </div>
        ) : error ? (
          <div className="memo-empty">
            ...
            <h5>{t("Could not load categories")}</h5>
          ...
          <div className="memo-empty">
            ...
            <h5>{t("No categories")}</h5>
            <p className="mb-0">{t("Create your first category to get started.")}</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <td>
                    <div className="d-flex gap-2">
                      <button className="memo-iconbtn" onClick={() => openEdit(row)} title={t("Edit")}>
                        <FaEdit size={12} />
                      </button>
                      <button className="memo-iconbtn memo-iconbtn--danger" onClick={() => remove(row)} title={t("Delete")}>
                        <FaTrash size={12} />
                      </button>
                    </div>
                  </td>
      ...
      >
        ...
        <div className="d-flex align-items-center gap-2 mt-3">
          ...
          <span className="text-muted small">{form.status === "active" ? t("Active") : t("Inactive")}</span>
        </div>
        <Group justify="flex-end" mt="lg">
          <Button variant="default" onClick={close} disabled={submitting}>{t("Cancel")}</Button>
          <Button onClick={submit} color="indigo" loading={submitting}>
            {form.id ? t("Save changes") : t("Create category")}
          </Button>
        </Group>
      </Modal>
    </div>
