  ...
  return (
    <Menu shadow="md" width={210} position="bottom-end" withinPortal>
      ...
      <Menu.Dropdown onClick={(e) => e.stopPropagation()}>
        <Menu.Item leftSection={<FaEye size={12} />} onClick={() => onAction("view", doc)}>{t("View Details")}</Menu.Item>
        <Menu.Item leftSection={<FaUpload size={12} />} onClick={() => onAction("upload", doc)}>{t("Upload New Version")}</Menu.Item>
        <Menu.Item leftSection={<FaUserShield size={12} />} onClick={() => onAction("access", doc)}>{t("Manage Access")}</Menu.Item>
        <Menu.Divider />
        {doc.status !== "published" && (
          <Menu.Item leftSection={<FaCheck size={12} />} color="green" onClick={() => onAction("publish", doc)}>{t("Publish")}</Menu.Item>
        )}
        {doc.status === "published" && (
          <Menu.Item leftSection={<FaArchive size={12} />} color="orange" onClick={() => onAction("archive", doc)}>{t("Archive")}</Menu.Item>
        )}
        <Menu.Item leftSection={<FaTrash size={12} />} color="red" onClick={() => onAction("delete", doc)}>{t("Delete")}</Menu.Item>
      </Menu.Dropdown>
    </Menu>
  );
};
    ...
    if (key === "publish") {
      setConfirm({
        ...
        action: async () => {
          ...
          notifySuccess(t("Memo published"));
        },
      });
    }
    if (key === "archive") {
      setConfirm({
        ...
        body: t('"{{title}}" will be hidden from end users.', { title: doc.title }),
        color: "orange",
        confirmLabel: t("Archive"),
        action: async () => {
          await archiveDocument(doc.id);
          notifySuccess(t("Memo archived"));
        },
      });
    }
    if (key === "delete") {
      setConfirm({
        title: t("Delete memo"),
        body: t('"{{title}}" will be permanently removed.', { title: doc.title }),
        color: "red",
        confirmLabel: t("Delete"),
        action: async () => {
          await deleteDoc(doc.id);
          notifySuccess(t("Memo deleted"));
        },
      });
    }
  };
    ...
    <div className="memo-page">
      ...
      <div className="memo-filterbar">
        <div className="memo-filterbar__search">
          ...
          <input
            placeholder={t("Search title, description, tags...")}
            value={q}
      ...
      {loading && (
        <div className="memo-card text-center py-5">
          <Loader color="indigo" />
          <p className="text-muted small mt-2 mb-0">{t("Loading memos…")}</p>
        </div>
      )}
      ...
      {!loading && error && (
        <div className="memo-card memo-empty">
          ...
          <h5>{t("Could not load memos")}</h5>
          <p className="mb-3">{error.message || t("Please try again.")}</p>
      ...
      {!loading && !error && paged.length === 0 && (
        <div className="memo-card memo-empty">
          ...
          <h5>{t("No memos found")}</h5>
          <p className="mb-3">{t("Try adjusting your filters or create a new memo.")}</p>
          <button className="memo-pillbtn memo-pillbtn--primary" onClick={createH.open}>
            <FaPlus size={11} /> {t("Create Memo")}
          </button>
        </div>
      ...
      {filtered.length > pageSize && (
        <div className="d-flex justify-content-between align-items-center mt-3">
          <div className="d-flex align-items-center gap-2">
            <span className="text-muted small">{t("Rows per page:")}</span>
            <select
          ...
          <div className="d-flex align-items-center gap-2">
            ...
            <span className="text-muted small">{t("Page {{safePage}} of {{totalPages}}", { safePage, totalPages })}</span>
            <button
              className="memo-pillbtn"
      ...
      >
        ...
        <Group justify="flex-end">
          <Button variant="default" onClick={() => setConfirm(null)} disabled={busy}>{t("Cancel")}</Button>
