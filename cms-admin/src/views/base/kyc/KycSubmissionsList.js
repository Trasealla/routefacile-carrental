...
const KycSubmissionsList = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!ok) {
        if (httpStatus === 403) {
          notifyError(t("You do not have permission to view KYC submissions."));
        } else {
          notifyError(data?.message || t("Failed to load KYC submissions"));
        }
        setItems([]);
        setLoading(false);
        return;
    ...
    <div className="hr-module kyc-module">
        ...
        <Row className="align-items-center g-3">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4>{t("KYC Submissions")}</h4>
                <p>
                  {t("Review identity verification requests, inspect supporting documents and confirm submission status")}
                </p>
                <div className="hr-hero-pills">
                  <span className="hr-hero-pill">
                    <FaInbox /> {t("{{count}} total", { count: globalStats.total })}
                  </span>
                  <span className="hr-hero-pill">
                    <FaPaperPlane /> {t("{{count}} submitted", { count: globalStats.submitted })}
                  </span>
                  <span className="hr-hero-pill">
                    <FaHourglassHalf /> {t("{{count}} under review", { count: globalStats.under_review })}
                  </span>
                  <span className="hr-hero-pill">
                    <FaThumbsUp /> {t("{{count}} approved", { count: globalStats.approved })}
                  </span>
                </div>
              </div>
            </div>
        ...
        <Row className="g-3 align-items-center">
          <Col xl={4} md={6}>
            <div className="kyc-input-group">
              ...
              <input
                type="text"
                placeholder={t("Search reference token, email, mobile or company...")}
                value={searchInput}
        ...
        <Row className="g-3 mt-1">
          <Col className="d-flex justify-content-end">
            <button
              ...
              onClick={exportCsv}
              disabled={exporting}
              title={t("Export current filters as CSV")}
            >
              {exporting ? (
                <>
                  <Spinner animation="border" size="sm" /> {t("Exporting…")}
                </>
              ) : (
                <>
                  <FaFileCsv size={12} /> {t("Export CSV")}
                </>
              )}
            </button>
          </Col>
      ...
      >
          ...
          <table className="hr-table">
            <thead>
              <tr>
                ...
                <th>{t("Company")}</th>
                <th>{t("Status")}</th>
                <th className="text-center">{t("Verification")}</th>
                ...
                <tr>
                  <td colSpan={10}>
                    <div className="kyc-empty">
                      ...
                      <p>{t("No KYC submissions match your filters.")}</p>
                    </div>
                  </td>
                </tr>
              ) : (
                    ...
                    >
                      ...
                      <td className="text-center">
                        {(() => {
                          ...
                          if (!sub.attachments) {
                            return (
                              <span className="kyc-docs-pill muted" title={t("Open submission to inspect documents")}>
                                —
                              </span>
                            );
                          }
                            ...
                            <span
                              ...
                              title={
                                docs.complete
                                  ...
                                  : t("One or more required documents missing")
                              }
                            >
                              {docs.complete ? (
                                <FaCheckCircle size={10} />
        ...
        <div className="kyc-pagination-footer">
          <div className="info">
            {t("Showing page")} <strong>{pagination.current_page || page}</strong> {t("of")}{" "}
            <strong>{pagination.total_pages || 1}</strong> ·{" "}
            <strong>{pagination.total_items || 0}</strong> {t("submissions")}
          </div>
