...
const KycSubmissionDetail = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!ok) {
        if (httpStatus === 403) {
          notifyError(t("You do not have permission to view this submission."));
        } else if (httpStatus === 404) {
          notifyError(t("Submission not found."));
        } else {
          notifyError(resp?.message || t("Failed to load submission"));
        }
        setData(null);
        setLoading(false);
        return;
  ...
  const downloadAttachment = async (att) => {
    ...
    try {
      ...
      if (!res.ok) {
        notifyError(t('Download failed ({{status}})', { status: res.status }));
        return;
      }
  ...
  const previewAttachment = async (att) => {
    ...
    try {
      ...
      const res = await fetch(url, { method: "GET", headers: authHeaders() });
      if (!res.ok) {
        notifyError(t('Preview failed ({{status}})', { status: res.status }));
        return;
      }
      const blob = await res.blob();
      const objUrl = URL.createObjectURL(blob);
      ...
      setPreviewName(att.original_name || att.file_name || t("Attachment"));
      openPreview();
    } catch (e) {
      notifyError(t("Preview failed"));
    } finally {
  ...
  const submitReview = async () => {
    ...
    if (cfg.requireRejection && !rejectionReason.trim()) {
      ...
      setReviewError(t("Rejection reason is required."));
      return;
    }
    ...
    if (!ok) {
      if (status === 403) {
        setReviewError(t("You do not have permission to perform this action."));
      } else {
        setReviewError(resp?.message || t("Failed to update submission status."));
      }
      return;
    }
    ...
    setData(merged);
    notifySuccess(t(cfg.successMessage));
    closeReview();
    setReviewAction(null);
  };
    ...
    return (
      <div className="hr-module kyc-module">
        <div className="hr-loading">
          ...
          <span className="ms-2 text-muted">{t('Loading submission...')}</span>
        </div>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="hr-module kyc-module">
        ...
        >
          <FaArrowLeft size={11} /> {t('Back to list')}
        </button>
        <div className="hr-glass-card text-center text-muted py-5">
          {t('Submission not available.')}
        </div>
      </div>
    );
  }
    ...
    <div className="hr-module kyc-module">
      ...
      >
        <Row className="align-items-center g-3">
          <Col>
            ...
            >
              <FaArrowLeft size={10} /> {t('Back to submissions')}
            </button>
            <div className="d-flex align-items-center">
              ...
              <div style={{ flex: 1 }}>
                <h4>
                  {data.company_name || data.email || t('KYC Submission')}
                </h4>
                <p>
                  {t('Reference token')}{" "}
                  <span
                ...
                <div className="hr-hero-pills">
                  ...
                  <span className="hr-hero-pill">
                    <FaPhoneAlt />{" "}
                    {data.phone_verified ? t("Phone OK") : t("Phone unverified")}
                  </span>
                  <span className="hr-hero-pill">
                    <FaEnvelope />{" "}
                    {data.email_verified ? t("Email OK") : t("Email unverified")}
                  </span>
                  {data.submitted_at ? (
                    <span className="hr-hero-pill">
                ...
                <div className="kyc-review-actions">
                  ...
                  <button
                    ...
                    title={t('Download submission as PDF')}
                  >
                    {pdfDownloading ? (
                      <>
                        <Spinner animation="border" size="sm" /> {t('Preparing…')}
                      </>
                    ) : (
                      <>
                        <FaFilePdf /> {t('Download PDF')}
                      </>
                    )}
                  </button>
                </div>
                  ...
                  <div className="kyc-sms-status mt-2">
                    {data.sms_status_error ? (
                      <span className="kyc-sms-status-pill red">
                        <FaTimesCircle size={11} /> {t('SMS failed:')}{" "}
                        {data.sms_status_error}
                      </span>
                    ) : (
                      <span className="kyc-sms-status-pill green">
                        <FaSms size={11} /> {t('SMS sent ·')}{" "}
          ...
          >
            <div className="hr-stat-card blue">
              ...
              <div>
                ...
                <div className="hr-stat-label">{t('Submission ID')}</div>
              </div>
            </div>
          </motion.div>
        </Col>
          ...
          >
            ...
            >
              ...
              <div>
                ...
                <div className="hr-stat-label">{t('Status')}</div>
              </div>
            </div>
          </motion.div>
        </Col>
          ...
          >
            <div className="hr-stat-card cyan">
              ...
              <div>
                ...
                <div className="hr-stat-label">{t('Attachments')}</div>
              </div>
            </div>
          </motion.div>
        </Col>
          ...
          >
            <div className="hr-stat-card violet">
              ...
              <div>
                ...
                <div className="hr-stat-label">{t('Verifications')}</div>
              </div>
            </div>
          </motion.div>
        </Col>
      ...
      >
        <SectionTitle
          ...
          sub={t('Identifiers and submission timeline')}
        />
        <Row>
          <Field
            icon={<FaIdBadge />}
            label={t('Reference Token')}
            value={
              data.reference_token ? (
      ...
      >
        <SectionTitle
          icon={<FaUserEdit />}
          title={t('Review')}
          sub={t('Decision history and internal notes')}
        />
            ...
            <Field
              ...
              value={
                ...
                (data.reviewed_by_admin_id
                  ? t('Admin #{{id}}', { id: data.reviewed_by_admin_id })
                  : null)
              }
            />
            <Field
      ...
      >
        <SectionTitle
          icon={<FaUserShield />}
          title={t('Personal & Residential')}
          sub={t('Applicant contact details')}
        />
        <Row>
          <Field
            icon={<FaMapMarkerAlt />}
      ...
      >
        <SectionTitle
          ...
          sub={t('Business details on record')}
        />
      ...
      >
        <SectionTitle
          ...
          sub={t('Phone and email verification status')}
        />
        <Row>
          <Col md={6} className="mb-3">
            <div className="kyc-field h-100">
              <div className="kyc-field-label">
                <FaPhoneAlt /> {t('Phone Verified')}
              </div>
              <div className="kyc-field-value">
                <span
                  className={`kyc-chip ${
                ...
                >
                  {data.phone_verified ? (
                    <>
                      <FaCheckCircle size={10} /> {t('Verified')}
                    </>
                  ) : (
                    <>
                      <FaTimesCircle size={10} /> {t('Not verified')}
                    </>
                  )}
                </span>
                {data.phone_verified_at ? (
                  ...
                  >
                    {t('Verified at {{date}}', { date: formatDateTime(data.phone_verified_at) })}
                  </div>
                ) : null}
              </div>
            </div>
            ...
            <div className="kyc-field h-100">
              ...
              <div className="kyc-field-value">
                ...
                {data.email_verified_at ? (
                  ...
                  >
                    {t('Verified at {{date}}', { date: formatDateTime(data.email_verified_at) })}
                  </div>
                ) : null}
              </div>
            </div>
      ...
      >
        <SectionTitle
          icon={<FaSignature />}
          title={t('Consent')}
          sub={t('Applicant agreement details')}
        />
        <Row>
          <Col md={12} className="mb-3">
            <div className="kyc-field-label mb-2">
              <FaFileAlt /> {t('Consent Text')}
            </div>
            {data.consent_text ? (
      ...
      >
        <SectionTitle
          icon={<FaSignature />}
          title={t('Signature')}
          sub={t('Customer-signed AECB acknowledgement')}
        />
        {!(
          data.signature_signed_at ||
          data.signature_method ||
              ...
              <div className="kyc-signature-frame">
                {signatureLoading ? (
                  ...
                  <div className="kyc-signature-loading text-danger">
                    <FaExclamationTriangle /> {t('Failed to load signature')}
                  </div>
                ) : signatureUrl ? (
                  <img
                    src={signatureUrl}
                ...
                <div className="kyc-signature-caption">
                  {t('Signed on {{date}}', { date: formatDateTime(data.signature_signed_at) })}
                </div>
              </div>
            </Col>
            <Col md={6}>
              <Row>
                ...
                <Col md={12} className="mb-3">
                  <div className="kyc-field h-100">
                    <div className="kyc-field-label">
                      <FaUserShield /> {t('Device / User Agent')}
                    </div>
                    <div
                      className="kyc-field-value"
                      style={{
          ...
          return (
            <>
              <SectionTitle
                ...
                sub={
                  <>
                    {t('{{filled}} / {{total}} required documents', { filled: completion.filled, total: completion.total })}
                    {" · "}
                    <span
                    ...
                    >
                      {completion.complete ? (
                        <>
                          <FaCheckCircle size={10} /> {t('Complete')}
                        </>
                      ) : (
                        <>
                          <FaExclamationTriangle size={10} /> {t('Incomplete')}
                        </>
                      )}
                    </span>
                  </>
                        ...
                        <div className="kyc-attachment-meta">
                          <p className="title">
                            ...
                            {legacy && (
                              <span
                                ...
                                title={t('Uploaded as a single file before front/back split')}
                              >
                                {t('Legacy single-side upload')}
                              </span>
                            )}
                            {!att && slot.required && (
                          ...
                          {att ? (
                            ...
                            <div className="subtitle muted">
                              {slot.required
                                ? t("No file uploaded for this side.")
                                : t("Customer chose not to upload this document.")}
                            </div>
                          )}
                        </div>
                        {att ? (
      ...
      <Modal
        ...
        title={
          <div style={{ fontWeight: 700, color: "#0f172a" }}>
            ...
            {reviewAction ? t(ACTION_CONFIG[reviewAction].title) : t('Review')}
          </div>
        }
      >
        <div className="kyc-modal-summary">
          ...
          <div>
            <strong>{t('Mobile:')}</strong> {renderMobile(data) || "—"}
          </div>
        </div>
        ...
        {reviewAction === "rejected" && (
          <div className="mb-3">
            <label className="kyc-modal-label">
              {t('Reason for rejection')} <span style={{ color: "#b91c1c" }}>*</span>
            </label>
            <textarea
              ...
              placeholder={t('Explain why this submission is being rejected (visible internally)')}
              disabled={reviewSubmitting}
            />
            <div className="kyc-modal-helper">
              <span>{t('Required. Shown on the detail page.')}</span>
        ...
        <div className="mb-3">
          <label className="kyc-modal-label">{t('Internal notes (optional)')}</label>
          <textarea
            className="kyc-modal-textarea"
            maxLength={2000}
            value={reviewNotes}
            onChange={(e) => setReviewNotes(e.target.value)}
            placeholder={t('Anything the team should know about this decision')}
            disabled={reviewSubmitting}
          />
          <div className="kyc-modal-helper">
            <span>{t('Visible to admin / KYC officers only.')}</span>
          ...
          >
            {reviewSubmitting ? (
              ...
              <>
                ...
                {reviewAction
                  ? t(ACTION_CONFIG[reviewAction].confirmLabel)
                  : t('Confirm')}
              </>
            )}
          </button>
