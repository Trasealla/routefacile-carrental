...
const ApplicationDetail = () => {
    ...
    simpleGetCallAuth(url)
      .then((res) => {
        if (res && !res.error) {
          ...
          notifyError(res?.message?.[0] || t("Failed to load application"));
        }
      })
      .catch(() => {
    ...
    simplePutCallAuth(url, body)
      .then((res) => {
        if (res?.status === "success") {
          notifySuccess(t("Application updated successfully"));
          getDetails();
        } else {
          notifyError(
            Array.isArray(res?.message)
              ? res.message[0]
              : res?.message || t("Update failed")
          );
        }
      })
      .catch(() => {
  ...
  const handleDownloadCV = () => {
    ...
    fetch(url, {
      ...
      .catch(() => {
        notifyError(t("Failed to download CV"));
      });
  };
        ...
        <div className="career-empty-state">
          ...
          <Link to="/career/applications">
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
            >
              <FaArrowLeft size={13} /> {t("Back to Applications")}
            </motion.button>
          </Link>
        </div>
      </div>
  ...
  const infoItems = [
    { icon: <FaUser />, label: t("Full Name"), value: `${application.first_name || ""} ${application.last_name || ""}`.trim() || null },
    { icon: <FaEnvelope />, label: t("Email"), value: application.email },
    { icon: <FaPhone />, label: t("Phone"), value: application.phone_number ? `+${application.phone_code || ""} ${application.phone_number}` : null },
    { icon: <FaCalendarAlt />, label: t("Applied Date"), value: application.created_at ? new Date(application.created_at).toLocaleDateString() : null },
    {
      icon: <FaBriefcase />,
      label: t("Job Title"),
        ...
        <Row className="align-items-center">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4 className="career-header-title">{t("Application Detail")}</h4>
                <p className="career-header-subtitle">
                  {t("{{name}} — applied for {{job}}", {
                    name: `${application.first_name || ""} ${application.last_name || ""}`.trim(),
                    job: application.career_job?.title_en || t("N/A"),
                  })}
                </p>
              </div>
            </div>
          ...
          >
            <div className="d-flex align-items-center mb-4">
              ...
              <div className="ms-3">
                ...
                >
                  {t(STATUS_MAP[application.status]?.label || "N/A")}
                </span>
              </div>
            </div>
            ...
            >
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
              >
                <FaFileDownload size={16} /> {t("Download CV")}
              </motion.button>
            </motion.div>
          </motion.div>
        </Col>
          ...
          >
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                placeholder={t("Enter reviewer name")}
                style={{ borderRadius: "10px" }}
              />
            </Form.Group>
