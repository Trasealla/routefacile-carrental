...
const CareerJobs = () => {
    ...
    simpleDeleteCallAuth(url)
      .then((res) => {
        if (res?.status === "success") {
          ...
          notifyError(res?.message?.[0] || t("Delete failed"));
        }
      })
      .catch(() => {
        notifyError(t("Something went wrong. Please try again later."));
  ...
  const handleToggleStatus = (row) => {
    ...
    simplePutCallAuth(url, JSON.stringify({ status: newStatus }))
      .then((res) => {
        if (res?.status === "success") {
          notifySuccess(t("Status updated"));
          getJobList();
        } else {
          notifyError(res?.message?.[0] || t("Status update failed"));
        }
      })
      .catch(() => {
        notifyError(t("Something went wrong. Please try again later."));
    ...
    <div className="career-module">
        ...
        <Row className="align-items-center">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4 className="career-header-title">{t("Career Jobs")}</h4>
                <p className="career-header-subtitle">
                  {t("Manage job listings, toggle availability, and track openings")}
                </p>
              </div>
            </div>
          </Col>
      ...
      <motion.div variants={staggerContainer} initial="hidden" animate="visible">
        <Row className="mb-4 px-2">
          {[
            { label: t("Total Jobs"), value: jobList.length, icon: <FaBriefcase />, color: "blue" },
            { label: t("Active"), value: activeCount, icon: <FaCheckCircle />, color: "green" },
            { label: t("Inactive"), value: inactiveCount, icon: <FaTimesCircle />, color: "red" },
          ].map((stat, i) => (
      ...
      >
        <Row className="align-items-end g-3">
          <Col lg={3}>
            <Form.Label className="small fw-semibold text-secondary mb-1">{t("Status Filter")}</Form.Label>
            <Form.Select
              value={status}
          ...
          <Col lg={4}>
            <Form.Label className="small fw-semibold text-secondary mb-1">{t("Quick Search")}</Form.Label>
            <InputGroup>
              ...
              <Form.Control
                ...
                type="text"
                placeholder={t("Search title, location, or ID...")}
                value={searchTerm}
          ...
          <Col lg="auto" className="ms-auto">
            <div className="d-flex align-items-center gap-2">
              <span className="small text-muted fw-medium">{t("Show")}</span>
              <Form.Select
        ...
        <div className="career-table-card">
          {loading ? (
            ...
            <div className="career-empty-state">
              ...
              <p>{t("No jobs found. Create your first job listing!")}</p>
            </div>
          ) : (
            <>
              <div className="career-list-header">
                <span className="career-list-col career-list-col-id">#</span>
                <span className="career-list-col career-list-col-title">{t("Job Title")}</span>
                <span className="career-list-col career-list-col-location">{t("Location")}</span>
                <span className="career-list-col career-list-col-exp">{t("Experience")}</span>
                <span className="career-list-col career-list-col-expiry">{t("Expiry Date")}</span>
                <span className="career-list-col career-list-col-status">{t("Status")}</span>
                <span className="career-list-col career-list-col-actions">{t("Actions")}</span>
              </div>
                    ...
                    <span className="career-list-col career-list-col-title">
                      <div className="career-row-title-wrap">
                        ...
                        <div>
                          ...
                          {row.title_ar && (
                            <div className="career-row-title-ar" dir="rtl">{row.title_ar}</div>
                          )}
                        </div>
                      </div>
                    </span>
              ...
              {totalRecords > 0 && (
                <div className="career-pagination-wrap">
                  <span className="career-pagination-info">
                    {t("Showing {{from}}–{{to}} of {{total}} jobs", { from: startIndex + 1, to: Math.min(startIndex + pageSize, totalRecords), total: totalRecords })}
                  </span>
                  <CustomPagination
      ...
      <Modal show={showdel} onHide={handleCloseDel} centered className="career-delete-modal">
        <Modal.Header closeButton>
          <Modal.Title className="d-flex align-items-center gap-2">
            <FaExclamationTriangle className="text-danger" /> {t("Delete Job")}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {t("Are you sure you want to delete this job listing? This action cannot be undone.")}
        </Modal.Body>
        <Modal.Footer>
          ...
          <Button className="career-btn-danger" onClick={confirmDelete} disabled={deleteLoading}>
            {deleteLoading ? <Spinner size="sm" /> : t("Delete Job")}
          </Button>
        </Modal.Footer>
      </Modal>
    </div>
