...
const CreateLocation = () => {
  ...
  const validate = () => {
    ...
    if (!formData.name_en) newErrors.name_en = t("Name (EN) is required.");
    if (!formData.name_ar) newErrors.name_ar = t("Name (AE) is required.");
    if (!formData.address_en)
      newErrors.address_en = t("Address (EN) is required.");
    if (!formData.address_ar)
      newErrors.address_ar = t("Address (AE) is required.");
    if (!formData.recipients) newErrors.recipients = t("Recipients is required.");
    // if (!formData.status) newErrors.status = "Status is required.";
    // if (!formData.is_virtual) newErrors.is_virtual = "required.";
    if (!formData.timing_detail_en)
      newErrors.timing_detail_en = t("Timing Detail (EN) is required.");
    if (!formData.timing_detail_ar)
      newErrors.timing_detail_ar = t("Timing Detail (AE) is required.");
    if (formData.order === "" || formData.order === null)
      newErrors.order = t("Order is required.");
    if (formData.buffer_hours === "" || formData.buffer_hours === null)
      newErrors.buffer_hours = t("Buffer Hours are required.");
    ...
    if (!formData.lat) newErrors.lat = t("Latitude is required.");
    if (!formData.long) newErrors.long = t("Longitude is required.");
    if (!formData.contact_number)
      newErrors.contact_number = t("Contact Number is required.");
    if (!formData.city_id) newErrors.city_id = t("City ID is required.");

    formData.location_opening_hours.forEach((day) => {
      if (!day.shifts[0].from && !day.shifts[0]?.is_closed)
        newErrors[`day${day.day}_shift${day.shifts[0].shift}_from`] = t("Required");
      if (!day.shifts[0].to && !day.shifts[0]?.is_closed)
        newErrors[`day${day.day}_shift${day.shifts[0].shift}_to`] = t("Required");
    });
    setErrors(newErrors);
  ...
  const handleFormSubmit = () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      const body = JSON.stringify({
        ...
        timing_detail_ar: formData.timing_detail_ar,
        ...
        .then((res) => {
          if (res?.status === "success") {
            ...
            if (Array.isArray(res?.message)) {
              notifyError(res?.message[0] || t("Somthing is wrong."));
            } else {
              notifyError(res?.message || t("Somthing is wrong."));
            }
            ...
            (async () => {
              ...
              try {
                ...
                setFormData((prevData) => ({
                  ...
                  timing_detail_ar: res?.timing_detail_ar,
  ...
  const handleRemoveRow = async (index) => {
    ...
    if (row.id) {
      const confirmed = window.confirm(
        t("Are you sure you want to delete this exception hour?")
      );
      if (!confirmed) return;
      ...
      try {
        ...
        if (res?.status === "success") {
          ...
          notifyError(res?.message?.[0] || t("Could not delete record."));
        }
      } catch (err) {
        console.error(err);
  ...
  const handleExceptionHourSubmit = async (e, index) => {
    ...
    try {
      ...
      if (res?.status === "success") {
        ...
        notifyError(res?.message?.[0] || t("Something went wrong"));
      }
    } catch (error) {
      console.error(error);
    ...
    <Container className="container">
      <div className="d-flex justify-content-between align-items-center mt-3 mb-3">
        <Breadcrumb style={{ fontSize: "14px", marginBottom: 0 }}>
          <Breadcrumb.Item linkAs={Link} linkProps={{ to: "/" }}>{t("Dashboard")}</Breadcrumb.Item>
          <Breadcrumb.Item linkAs={Link} linkProps={{ to: "/cms/locations" }}>{t("Locations")}</Breadcrumb.Item>
          <Breadcrumb.Item active>{id ? t("Edit Location") : t("Create Location")}</Breadcrumb.Item>
        </Breadcrumb>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <Card className="mb-4" style={{ border: "1px solid #e0e0e0", borderRadius: "12px", overflow: "hidden" }}>
            <Card.Header style={{ background: "linear-gradient(135deg, #4b6cb7, #182848)", color: "#fff", padding: "14px 24px" }}>
              <h6 className="mb-0" style={{ fontWeight: 600 }}>{t("Basic Information")}</h6>
            </Card.Header>
            <Card.Body style={{ padding: "24px" }}>
              <Row>
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="name_en" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Name (EN)")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="name_ar" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Name (AE)")}</Form.Label>
                    <Form.Control type="text" name="name_ar" value={formData.name_ar} onChange={handleChange} isInvalid={!!errors.name_ar} style={{ borderRadius: "8px" }} />
                    <Form.Control.Feedback type="invalid">{errors.name_ar}</Form.Control.Feedback>
                  </Form.Group>
                </Col>
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="address_en" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Address (EN)")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="address_ar" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Address (AE)")}</Form.Label>
                    <Form.Control type="text" name="address_ar" value={formData.address_ar} onChange={handleChange} isInvalid={!!errors.address_ar} style={{ borderRadius: "8px" }} />
                    <Form.Control.Feedback type="invalid">{errors.address_ar}</Form.Control.Feedback>
                  </Form.Group>
                </Col>
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="city_id" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("City")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="status" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Status")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="is_virtual" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Virtual Location")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="order" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Order")}</Form.Label>
          ...
          <Card className="mb-4" style={{ border: "1px solid #e0e0e0", borderRadius: "12px", overflow: "hidden" }}>
            <Card.Header style={{ background: "linear-gradient(135deg, #2d8659, #1a5c3a)", color: "#fff", padding: "14px 24px" }}>
              <h6 className="mb-0" style={{ fontWeight: 600 }}>{t("Service & Contact")}</h6>
            </Card.Header>
            <Card.Body style={{ padding: "24px" }}>
              <Row>
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="pickup" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Pickup")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="dropoff" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Dropoff")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="buffer_hours" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Buffer Hours")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="parking_charges" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Parking Charges")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="contact_number" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Contact Number")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={8} xl={6}>
                  <Form.Group controlId="recipients" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Recipients")}</Form.Label>
                    <Form.Control type="text" placeholder={t("Add multiple emails separated by commas")} name="recipients" value={formData.recipients} onChange={handleChange} isInvalid={!!errors.recipients} style={{ borderRadius: "8px" }} />
          ...
          <Card className="mb-4" style={{ border: "1px solid #e0e0e0", borderRadius: "12px", overflow: "hidden" }}>
            <Card.Header style={{ background: "linear-gradient(135deg, #c0392b, #8e2420)", color: "#fff", padding: "14px 24px" }}>
              <h6 className="mb-0" style={{ fontWeight: 600 }}>{t("Location & Timing Details")}</h6>
            </Card.Header>
            <Card.Body style={{ padding: "24px" }}>
              <Row>
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="lat" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Latitude")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="long" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Longitude")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="timing_detail_en" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Timing Detail (EN)")}</Form.Label>
                ...
                <Col xs={12} md={6} lg={4} xl={3}>
                  <Form.Group controlId="timing_detail_ar" className="mb-3">
                    <Form.Label style={{ fontSize: "13px", fontWeight: 600, color: "#555" }}>{t("Timing Detail (AE)")}</Form.Label>
                    <Form.Control type="text" name="timing_detail_ar" value={formData.timing_detail_ar} onChange={handleChange} isInvalid={!!errors.timing_detail_ar} style={{ borderRadius: "8px" }} />
                    <Form.Control.Feedback type="invalid">{errors.timing_detail_ar}</Form.Control.Feedback>
                  </Form.Group>
                </Col>
              </Row>
            </Card.Body>
          ...
          <Card className="mb-4" style={{ border: "1px solid #e0e0e0", borderRadius: "12px", overflow: "hidden" }}>
            <Card.Header style={{ background: "linear-gradient(135deg, #e67e22, #d35400)", color: "#fff", padding: "14px 24px" }}>
              <h6 className="mb-0" style={{ fontWeight: 600 }}>{t("Weekly Opening Hours")}</h6>
            </Card.Header>
                            ...
                            <Col xs={6}>
                              <Form.Group controlId={`day${day.day}_shift${shift.shift}_from`}>
                                ...
                                >
                                  ...
                                  <option value="is_closed">{t("Closed")}</option>
                                  {timings?.map((time, i) => (
          ...
          <div className="d-flex justify-content-end mb-4">
            ...
            >
              {loading ? <Spinner size="sm" /> : id ? t("Update Location") : t("Create Location")}
            </Button>
          </div>
        </Form>
      )}
        ...
        >
          <div>
            ...
            <small style={{ opacity: 0.8 }}>
              {t("Manage special operating hours for holidays, Ramadan, and other events")}
            </small>
          </div>
          <Button
            variant="light"
        ...
        <Card.Body style={{ padding: "24px", backgroundColor: "#fafbfc" }}>
          {exception_hours?.length === 0 && (
            ...
            >
              ...
              <p className="mb-0">{t("No exception hours configured")}</p>
              <small>{t('Click "+ Add Exception" to set special operating hours')}</small>
            </div>
          )}
