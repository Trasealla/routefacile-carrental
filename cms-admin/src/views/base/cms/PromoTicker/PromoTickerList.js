    ...
    simpleDeleteCallAuth(configWeb.DELETE_PROMO_TICKER(id))
      .then((res) => {
        if (res?.status === "success") {
          notifySuccess(t("Promo ticker deleted"));
          getTickerList();
        } else {
          notifyError(res?.message?.[0] || t("Failed to delete"));
        }
      })
  ...
  const toggleStatus = async (row) => {
    ...
    try {
      ...
      if (res?.status === "success") {
        ...
        notifyError(res?.message?.[0] || t("Failed to update status"));
      }
    } catch {
      notifyError(t("Something went wrong. Please try again later."));
    }
    ...
    if (searchTerm) {
      ...
      list = list.filter(
        (r) =>
          ...
          (r.text_ar && r.text_ar.includes(searchTerm)) ||
  ...
  return (
    <Container fluid className="px-lg-4 pb-5">
      ...
      <div className="promo-ticker-page-header d-flex flex-wrap align-items-center justify-content-between mt-3">
        <div>
          <h4>{t("Promo Ticker")}</h4>
          <p>{t("Manage the scrolling news bar displayed on the website")}</p>
        </div>
        ...
        <Card.Body>
          <Row className="align-items-end gx-3">
            ...
            <Col lg={3} md={6} className="mb-2">
              ...
              >
                <option value="asc">{t("Ascending (1, 2, 3…)")}</option>
                <option value="desc">{t("Descending (…3, 2, 1)")}</option>
              </Form.Select>
            </Col>
            <Col lg={3} md={6} className="mb-2">
              <Form.Label>{t("Quick Toggle")}</Form.Label>
              <div className="mt-1">
                <Form.Check
                  type="switch"
            ...
            <Col lg={3} md={6} className="mb-2">
              ...
              >
                <option value={10}>{t("{{n}} per page", { n: 10 })}</option>
                <option value={25}>{t("{{n}} per page", { n: 25 })}</option>
                <option value={50}>{t("{{n}} per page", { n: 50 })}</option>
                <option value={100}>{t("{{n}} per page", { n: 100 })}</option>
              </Form.Select>
            </Col>
          </Row>
        </Card.Body>
      ...
      <div className="promo-ticker-table-wrapper">
        ...
        <div className="p-3 promo-search-bar d-flex flex-wrap align-items-center justify-content-between gap-2">
          <InputGroup style={{ maxWidth: 360 }}>
            ...
            <Form.Control
              type="text"
              placeholder={t("Search text or ID…")}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </InputGroup>
          <span className="showing-text">
            {t("{{count}} ticker(s) found", { count: totalRecords })}
          </span>
        </div>
        ...
        <div className="table-responsive">
          <Table className="promo-ticker-table mb-0">
            <thead>
              <tr>
                <th style={{ width: "5%" }}>#</th>
                <th style={{ width: "30%" }}>{t("Text (EN)")}</th>
                <th style={{ width: "10%", textAlign: "center" }}>{t("Status")}</th>
                <th style={{ width: "18%" }}>{t("Date Range")}</th>
                <th style={{ width: "8%", textAlign: "center" }}>{t("Order")}</th>
                <th style={{ width: "10%", textAlign: "center" }}>{t("Speed")}</th>
                <th style={{ width: "12%", textAlign: "center" }}>{t("Actions")}</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={7}>
                    <div className="promo-empty-state">
                      ...
                      <h5>{t("No tickers found")}</h5>
                      <p>
                        {searchTerm || hasActiveFilters
                          ? t("Try adjusting your filters or search term.")
                          : t("Create your first promo ticker to get started.")}
                      </p>
                      {!searchTerm && !hasActiveFilters && (
                        <Link to="/cms/create-promo-ticker">
                          <Button
                          ...
                          >
                            <FaPlus className="me-1" /> {t("Add Ticker")}
                          </Button>
                        </Link>
                      )}
                    </div>
                    ...
                    >
                      ...
                      <td>
                        <div className="date-range-cell">
                          <div>
                            <span className="date-label">{t("From")} </span>
                            <span className="date-value">{formatDate(row.start_date)}</span>
                          </div>
                          <div>
                            <span className="date-label">{t("To")} </span>
                      ...
                      {/* Speed */}
                      <td className="text-center">
                        <span className="speed-badge" title={t("{{n}}s per cycle", { n: row.scroll_speed || 20 })}>
                          <FaTachometerAlt size={11} className="me-1" />
                          {t(getSpeedLabel(row.scroll_speed || 20))}
                        </span>
                      </td>
          ...
          <div className="promo-pagination-footer d-flex flex-wrap justify-content-between align-items-center gap-2">
            <span className="showing-text">
              {t("Showing {{start}}–{{end}} of {{total}}", {
                start: startIdx + 1,
                end: Math.min(startIdx + pageSize, totalRecords),
                total: totalRecords,
              })}
            </span>
            <CustomPagination
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title style={{ fontSize: "1.05rem" }}>
            ...
            {t("Delete Promo Ticker")}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="delete-icon-wrapper"><FaTrashAlt /></div>
          <h6 className="fw-bold mb-2">{t("Are you sure?")}</h6>
          <p className="text-muted mb-0" style={{ fontSize: "0.9rem" }}>
            {t("This ticker will be removed and will no longer appear on the website. This action cannot be undone.")}
          </p>
        </Modal.Body>
        <Modal.Footer>
          <Button
