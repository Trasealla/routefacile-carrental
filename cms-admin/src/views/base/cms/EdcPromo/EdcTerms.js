...
const EdcTerms = () => {
  ...
  const calculatePaginationMessage = () => {
    ...
    return t("Showing {{startRecord}} to {{endRecord}} of {{totalRecords}} entries", { startRecord, endRecord, totalRecords });
  };
    ...
    simpleDeleteCallAuth(url)
      .then((res) => {
        if (res?.status === "success") {
          notifySuccess(t("Term deleted successfully"));
          getTermsList();
        } else {
          notifyError(res?.message?.[0] || res?.message || t("Failed to delete term"));
        }
      })
      .catch((error) => {
  ...
  const handleSubmit = (event) => {
    ...
    apiCall(url, body)
      .then((res) => {
        if (res?.status === "success") {
          notifySuccess(editMode ? t("Term updated successfully") : t("Term created successfully"));
          handleCloseFormModal();
          getTermsList();
        } else {
          notifyError(res?.message?.[0] || res?.message || t("Failed to save term"));
        }
      })
      .catch((error) => {
  ...
  const handleReorder = (index, direction) => {
    ...
    simplePutCallAuth(url, body)
      .then((res) => {
        if (res?.status === "success") {
          notifySuccess(t("Order updated successfully"));
        } else {
          notifyError(t("Failed to update order"));
  ...
  return (
    <Container fluid>
      <Row className="mb-3">
        <Col>
          <h4 className="mb-0">{t("EDC Terms & Conditions")}</h4>
          <small className="text-muted">
            {t("Manage terms and conditions for EDC promotional offer")}
          </small>
        </Col>
        ...
        <Col lg="12">
          ...
          <Table className="table table-striped" style={{ width: "100%", tableLayout: "fixed" }}>
            <thead>
              <tr>
                <th style={{ width: "5%" }}>#</th>
                <th style={{ width: "35%" }}>{t("Term (English)")}</th>
                <th style={{ width: "35%" }}>{t("Term (Arabic)")}</th>
                <th style={{ width: "10%" }}>{t("Status")}</th>
                <th style={{ width: "15%" }} className="text-center">{t("Actions")}</th>
              </tr>
            </thead>
      ...
      <Modal show={showDeleteModal} onHide={handleCloseDeleteModal}>
        <Modal.Header closeButton>
          <Modal.Title>{t("Delete Term")}</Modal.Title>
        </Modal.Header>
        <Modal.Body>{t("Are you sure you want to delete this term?")}</Modal.Body>
        <Modal.Footer>
          <Button
            variant="danger"
      ...
      <Modal show={showFormModal} onHide={handleCloseFormModal} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>{editMode ? t("Edit Term") : t("Add New Term")}</Modal.Title>
        </Modal.Header>
        <Form noValidate validated={validated} onSubmit={handleSubmit}>
            ...
            <Row>
              <Col xs={12} className="mb-3">
                <Form.Group controlId="text_en">
                  <Form.Label>{t("Term (English)")}</Form.Label>
                  <Form.Control
                    as="textarea"
                    rows={2}
                    name="text_en"
                    ...
                    placeholder={t("e.g., Valid EDC Student ID required")}
                    required
                  />
                  <Form.Control.Feedback type="invalid">
                    {t("Please enter the term in English.")}
              ...
              <Col xs={12} className="mb-3">
                <Form.Group controlId="text_ar">
                  <Form.Label>{t("Term (Arabic)")}</Form.Label>
                  <Form.Control
                    as="textarea"
                    rows={2}
                    name="text_ar"
          ...
          <Modal.Footer>
            ...
            >
              {saveLoading ? <Spinner size="sm" /> : editMode ? t("Update Term") : t("Save Term")}
            </Button>
          </Modal.Footer>
        </Form>
      </Modal>
