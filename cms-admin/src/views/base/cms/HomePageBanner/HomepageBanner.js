    ...
    <Container>
      <Row>
        ...
        >
          <Link to="/cms/create-home-banner">
            <Button className="btn-def">{t("Add Banner")}</Button>
          </Link>
        </Col>
      </Row>
      ...
      <Card className="mt-2 mb-4 shadow-sm homepage-banner-filters">
        <Card.Header className="bg-light d-flex justify-content-between align-items-center">
          <h5 className="mb-0 d-flex align-items-center">
            ...
            {t("Filters & Sorting")}
            {hasActiveFilters && (
              <span className="ms-2 badge bg-primary">{t("Active")}</span>
            )}
          </h5>
          {hasActiveFilters && (
            <Button
            ...
            >
              ...
              <span className="ms-1">{t("Clear Filters")}</span>
            </Button>
          )}
        </Card.Header>
        <Card.Body>
          <Row>
            <Col lg="3">
              <Form.Group className="mb-3">
                <Form.Label className="fw-semibold">{t("Status Filter")}</Form.Label>
                <Form.Select
                  name="status"
                  value={status}
                  ...
                  className="shadow-sm"
                >
                  <option value="">{t("All Banners")}</option>
            ...
            <Col lg="3">
              <Form.Group className="mb-3">
                <Form.Label className="fw-semibold">{t("Quick Filters")}</Form.Label>
                <div className="mt-2">
                  <Form.Check
                    type="switch"
        ...
        <Row className="mb-3">
          <Col>
            <div className="alert alert-info py-2 d-flex align-items-center">
              ...
              <span className="ms-2">
                {showActiveOnly && t("Active Banners Only   • ")}
                {status === "1" && !showActiveOnly && t("Active Status   • ")}
                {status === "0" && t("Inactive Status (Status ≠ 1) • ")}
                {sortOrder === "desc" && t("Sorted by Order (Descending) • ")}
                {sortOrder === "asc" && t("Sorted by Order (Ascending)")}
              </span>
            </div>
          </Col>
        </Row>
        ...
        <Row className="mb-3">
          <Col lg="4">
            <InputGroup>
              ...
              <Form.Control
                type="text"
                placeholder={t("Search by Alt Text, URL, or ID...")}
                value={searchTerm}
          ...
          <Table striped bordered hover className="w-100">
            <thead className="table-light">
              <tr>
                ...
                <th style={{ width: "30%" }}>{t("Image")}</th>
                <th style={{ width: "15%" }}>{t("Alt Text")}</th>
                <th style={{ width: "10%" }}>{t("URL")}</th>
                  ...
                  <tr key={row.id}>
                    ...
                    <td className="text-center">
                      {row?.desktop ? (
                        ...
                        <span className="text-muted small">{t("No image")}</span>
                      )}
                    </td>
                    <td>{row.alt_text || ""}</td>
                    <td>
                    ...
                    <td className="text-center">
                      <div className={`px-4 py-2 font-size-14px d-flex align-items-center justify-content-center ${row.status === 1 ? "active_box" : "inactive_box"}`}>
                        {row.status === 1 ? (
                          <>
                            <span className="me-1">✓</span> {t("Active")}
                          </>
                        ) : (
                          <>
                            <span className="me-1">✗</span> {t("Inactive")}
                          </>
                        )}
                      </div>
                    </td>
      ...
      <Modal show={showdel} onHide={handleCloseDel}>
        <Modal.Header closeButton>
          <Modal.Title>{t("Delete Banner !")}</Modal.Title>
        </Modal.Header>
        <Modal.Body>{t("Are you sure you want to delete this banner ?")}</Modal.Body>
        <Modal.Footer>
          <Button
            className="btn-def"
