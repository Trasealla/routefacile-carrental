...
const HRInterviewDetail = () => {
  ...
  const load = () => {
    ...
    simpleGetCallAuth(configWeb.GET_RECRUITING_INTERVIEW_DETAILS(id))
      ...
      .catch(() => notifyError(t("Failed to load interview")))
  ...
  const handleSave = () => {
    ...
    simplePutCallAuth(configWeb.PUT_RECRUITING_INTERVIEW_UPDATE(id), body)
      .then((res) => {
        if (res && !res.error) {
          notifySuccess(t("Interview updated"));
          load();
        } else {
          notifyError(Array.isArray(res?.message) ? res.message[0] : res?.message || t("Update failed"));
        }
      })
  ...
  const handleReschedule = () => {
    if (!reschedDate) return notifyError(t("Select new date"));
    setSaving(true);
    const body = JSON.stringify({ interview_date: reschedDate, status: 4 });
    simplePutCallAuth(configWeb.PUT_RECRUITING_INTERVIEW_UPDATE(id), body)
      .then((res) => {
        if (res && !res.error) {
          notifySuccess(t("Interview rescheduled"));
          closeResched();
          load();
        } else notifyError(t("Reschedule failed"));
      })
      .catch(() => notifyError(t("Something went wrong")))
      .finally(() => setSaving(false));
  };
  ...
  if (!data) {
    return (
      <div className="hr-module">
        <div className="hr-empty-state"><FaCalendarCheck size={40} /><p>{t("Interview not found")}</p></div>
      </div>
    );
  }
  ...
  const candidateName = app ? getApplicantName(app) : t("App #{{id}}", { id: data.application_id });
  ...
  return (
    <div className="hr-module">
            ...
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4 style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  {t("Interview #{{id}}", { id: data.id })}
                  <span className="hr-badge" style={{ background: ist.bg, color: ist.text, fontSize: "0.72rem" }}>{t(ist.label)}</span>
                </h4>
                <p>
                  {t("with")} <strong>{candidateName}</strong> — {formatDateTime(data.interview_date)}
                </p>
              </div>
            </div>
          </Col>
          <Col xs="auto">
            <div className="d-flex gap-2 flex-wrap">
              {app && (
                ...
                >
                  <FaBriefcase className="me-1" /> {t("Application")}
                </motion.button>
              )}
              {data.status === 0 && (
                <motion.button
                  ...
                  whileHover={{ scale: 1.04 }}
                >
                  <FaClock className="me-1" /> {t("Reschedule")}
                </motion.button>
              )}
            </div>
          </Col>
      ...
      <Row className="mx-1 mt-3 g-3">
        ...
        <Col lg={7}>
          ...
          <motion.div className="hr-detail-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }}>
            <h6 className="fw-bold mb-3"><FaCalendarCheck className="me-2 text-primary" />{t("Interview Details")}</h6>
            <Row className="g-3">
              <Col md={6}>
                <div className="hr-info-item">
                  <div className="hr-info-icon"><FaClock /></div>
                  <div><small className="text-muted">{t("Date & Time")}</small><div className="fw-semibold">{formatDateTime(data.interview_date)}</div></div>
                </div>
              </Col>
              <Col md={6}>
                <div className="hr-info-item">
                  <div className="hr-info-icon">{interviewTypeIcon(data.interview_type)}</div>
                  <div><small className="text-muted">{t("Type")}</small><div className="fw-semibold" style={{ textTransform: "capitalize" }}>{data.interview_type || "—"}</div></div>
                </div>
              </Col>
              <Col md={6}>
                <div className="hr-info-item">
                  <div className="hr-info-icon"><FaMapMarkerAlt /></div>
                  <div><small className="text-muted">{t("Location")}</small><div className="fw-semibold">{data.location || "—"}</div></div>
                </div>
              </Col>
              <Col md={6}>
                <div className="hr-info-item">
                  ...
                  <div>
                    <small className="text-muted">{t("Interviewer")}</small>
            ...
            {data.notes && (
              <div className="mt-3 p-3" style={{ background: "#f8fafc", borderRadius: 10, border: "1px solid #e2e8f0" }}>
                <small className="text-muted d-flex align-items-center gap-1 mb-1"><FaStickyNote size={10} /> {t("Pre-Interview Notes")}</small>
            ...
            {data.feedback && (
              <div className="mt-2 p-3" style={{ background: "#f0fdf4", borderRadius: 10, border: "1px solid #bbf7d0" }}>
                <small className="text-muted d-flex align-items-center gap-1 mb-1"><FaStar size={10} /> {t("Feedback")}</small>
          ...
          {app && (
            <motion.div className="hr-detail-card mt-3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.4 }}>
              <h6 className="fw-bold mb-3"><FaUser className="me-2 text-primary" />{t("Candidate")}</h6>
              <div className="d-flex align-items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  {app.career_job?.title_en && <div style={{ fontSize: "0.82rem", color: "#64748b" }}>{t("Applied for:")} {app.career_job.title_en}</div>}
                </div>
              </div>
              <Row className="g-3">
                <Col md={6}>
                  <div className="hr-info-item">
                    ...
                    <div><small className="text-muted">{t("Email")}</small><div className="fw-semibold" style={{ fontSize: "0.85rem" }}>{app.email || "—"}</div></div>
                  </div>
                </Col>
                <Col md={6}>
                  <div className="hr-info-item">
                    <div className="hr-info-icon"><FaPhone /></div>
                    <div><small className="text-muted">{t("Phone")}</small><div className="fw-semibold" style={{ fontSize: "0.85rem" }}>{getApplicantPhone(app)}</div></div>
                  </div>
                </Col>
                <Col md={6}>
                  <div className="hr-info-item">
                    <div className="hr-info-icon"><FaBriefcase /></div>
                    <div><small className="text-muted">{t("Experience")}</small><div className="fw-semibold" style={{ fontSize: "0.85rem" }}>{app.experience_years ? t("{{years}} years", { years: app.experience_years }) : "—"}</div></div>
                  </div>
                </Col>
              </Row>
        ...
        <Col lg={5}>
          <motion.div className="hr-detail-card" style={{ position: "sticky", top: 20 }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.4 }}>
            <h6 className="fw-bold mb-3">{t("Update Interview")}</h6>

            <div className="hr-spotlight-card mb-3">
              <div style={{ fontSize: "0.72rem", fontWeight: 700, color: "#1d4ed8", textTransform: "uppercase", letterSpacing: "0.6px" }}>{t("Quick summary")}</div>
              <Row className="g-2 mt-1">
                <Col xs={4}>
                  <div className="hr-mini-metric" style={{ padding: 12 }}>
                    <div className="label">{t("Status")}</div>
                    <div className="sub" style={{ marginTop: 8, fontWeight: 700, color: "#0f172a" }}>{t(ist.label)}</div>
                  </div>
                </Col>
                <Col xs={4}>
                  <div className="hr-mini-metric" style={{ padding: 12 }}>
            ...
            <Form.Group className="mb-3">
              <Form.Label style={{ fontSize: "0.82rem", fontWeight: 600, color: "#475569" }}>{t("Status")}</Form.Label>
              ...
              <Col md={6}>
                <Form.Group>
                  ...
                  <Form.Select value={interviewType} onChange={(e) => setInterviewType(e.target.value)} style={{ borderRadius: 10 }}>
                    {INTERVIEW_TYPES.map((type) => <option key={type.value} value={type.value}>{t(type.label)}</option>)}
                  </Form.Select>
                </Form.Group>
              </Col>
            </Row>
            ...
            <Form.Group className="mb-3">
              <Form.Label style={{ fontSize: "0.82rem", fontWeight: 600, color: "#475569" }}>{t("Location")}</Form.Label>
            ...
            <Form.Group className="mb-3">
              <Form.Label style={{ fontSize: "0.82rem", fontWeight: 600, color: "#475569" }}>{t("Rating")}</Form.Label>
              <div>
            ...
            <Form.Group className="mb-3">
              <Form.Label style={{ fontSize: "0.82rem", fontWeight: 600, color: "#475569" }}>{t("Feedback")}</Form.Label>
              <Form.Control as="textarea" rows={3} value={feedback} onChange={(e) => setFeedback(e.target.value)} placeholder={t("Post-interview feedback...")} style={{ borderRadius: 10, resize: "vertical" }} />
            </Form.Group>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
            >
              {saving ? <Spinner size="sm" /> : t("Save Changes")}
            </motion.button>
          </motion.div>
        </Col>
      </Row>
      ...
      {/* Reschedule Modal */}
      <Modal opened={reschedOpen} onClose={closeResched} title={t("Reschedule Interview")} centered radius="lg" size="sm">
        <Form.Group className="mb-3">
          <Form.Label className="fw-semibold" style={{ fontSize: "0.82rem", color: "#475569" }}>{t("New Date & Time")}</Form.Label>
        ...
        <div className="d-flex justify-content-end gap-2 mt-3">
          <motion.button className="btn btn-sm" style={{ borderRadius: 10, padding: "8px 20px" }} onClick={closeResched}>{t("Cancel")}</motion.button>
          <motion.button
            className="btn btn-sm"
          ...
          >
            {saving ? <Spinner size="sm" /> : t("Reschedule")}
          </motion.button>
        </div>
      </Modal>
    </div>
