  ...
  useEffect(() => {
    ...
    simpleGetCallAuth(configWeb.GET_CAREER_JOB_DETAILS(id))
      .then((res) => {
        ...
        else notifyError(t("Failed to load job details"));
      })
        ...
        <Row className="align-items-center">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4>{job.title_en || t("Job #{{id}}", { id: job.id })}</h4>
                <p>{t("Job posting overview & AI screening setup")}</p>
              </div>
            </div>
          </Col>
          <Col xs="auto">
          ...
          <motion.div className="hr-detail-card" variants={fadeUp} initial="hidden" animate="visible" custom={0}>
            <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
              <h6 style={{ marginBottom: 0, display: "flex", alignItems: "center", gap: 8 }}>
                <FaBriefcase size={14} style={{ color: "#228be6" }} /> {t("Overview")}
              </h6>
              <div className="d-flex align-items-center gap-2">
                <span className="hr-badge" style={{ background: statusMeta.bg, color: statusMeta.text }}>{statusMeta.label}</span>
                {expired && <Badge size="sm" color="red" variant="light">{t("Expired")}</Badge>}
                {!expired && daysLeft != null && daysLeft <= 7 && (
                  <Badge size="sm" color="orange" variant="light">{t("Closing in {{days}}d", { days: Math.max(daysLeft, 0) })}</Badge>
                )}
              </div>
            </div>
            ...
            <Row className="g-3 mb-3">
              <Col md={4}>
                <div className="hr-mini-metric">
                  <div className="label"><FaMapMarkerAlt size={11} className="me-1" /> {t("Location")}</div>
                  <div className="value" style={{ fontSize: "1rem" }}>{job.location_en || "—"}</div>
                  {job.location_ar && <div className="sub" style={{ direction: "rtl" }}>{job.location_ar}</div>}
                </div>
              </Col>
              <Col md={4}>
                <div className="hr-mini-metric">
                  <div className="label"><FaClock size={11} className="me-1" /> {t("Experience")}</div>
                  <div className="value" style={{ fontSize: "1rem" }}>{job.experience_years != null ? t("{{years}} yrs", { years: job.experience_years }) : "—"}</div>
                  <div className="sub">{t("Required level")}</div>
                </div>
              </Col>
              <Col md={4}>
                <div className="hr-mini-metric">
                  <div className="label"><FaCalendarAlt size={11} className="me-1" /> {t("Expiry")}</div>
                  <div className="value" style={{ fontSize: "1rem", color: expired ? "#ef4444" : "#0f172a" }}>{formatDate(job.expiry_date)}</div>
                  <div className="sub">{expired ? t("Already expired") : daysLeft != null ? t("{{days}} days left", { days: Math.max(daysLeft, 0) }) : "—"}</div>
                </div>
              </Col>
            </Row>
            ...
            {(job.title_ar && job.title_ar !== job.title_en) && (
              <div className="mb-3">
                <div style={{ fontSize: "0.78rem", color: "#64748b", fontWeight: 600, marginBottom: 4 }}>{t("Title (AR)")}</div>
                <div style={{ direction: "rtl", color: "#0f172a", fontWeight: 600 }}>{job.title_ar}</div>
              </div>
            )}

            <div className="mb-3">
              <div style={{ fontSize: "0.78rem", color: "#64748b", fontWeight: 600, marginBottom: 6 }}>{t("Description (EN)")}</div>
              <div
                style={{ fontSize: "0.88rem", color: "#334155", lineHeight: 1.6 }}
                dangerouslySetInnerHTML={{ __html: job.description_en || `<em>${t("No description provided")}</em>` }}
              />
            </div>

            {job.description_ar && (
              <div>
                <div style={{ fontSize: "0.78rem", color: "#64748b", fontWeight: 600, marginBottom: 6 }}>{t("Description (AR)")}</div>
                <div
                  style={{ fontSize: "0.88rem", color: "#334155", lineHeight: 1.6, direction: "rtl" }}
                  dangerouslySetInnerHTML={{ __html: job.description_ar }}
                />
              </div>
            )}
          </motion.div>
        ...
        <Col lg={4}>
          <motion.div className="hr-detail-card" variants={fadeUp} initial="hidden" animate="visible" custom={1}>
            <h6 style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <FaUsers size={14} style={{ color: "#228be6" }} /> {t("At a glance")}
            </h6>
            <Row className="g-3 mt-1">
              <Col xs={12}>
                <div className="hr-mini-metric">
                  <div className="label">{t("Applications")}</div>
                  <div className="value">{applicantCount ?? "—"}</div>
                  <div className="sub">{t("Total received for this role")}</div>
                </div>
              </Col>
              <Col xs={12}>
                <div className="hr-mini-metric">
                  ...
                  <div className="value" style={{ fontSize: "1rem", display: "flex", alignItems: "center", gap: 8 }}>
                    {expired ? (
                      <><FaExclamationTriangle style={{ color: "#ef4444" }} /> {t("Expired")}</>
                    ) : job.status === 1 ? (
                      <><FaCheckCircle style={{ color: "#22c55e" }} /> {t("Active & live")}</>
                    ) : (
                      <><FaExclamationTriangle style={{ color: "#94a3b8" }} /> {t("Paused")}</>
                    )}
                  </div>
                  <div className="sub">{expired ? t("Renew to start collecting applicants") : t("Visible on careers page")}</div>
                </div>
              </Col>
              <Col xs={12}>
                <button
