...
const HRDashboard = () => {
  ...
  const statusChartData = (stats?.applications_by_status || []).map((s) => ({
    ...
    label: APPLICATION_STATUS[s.status]?.label || t("Status {{status}}", { status: s.status }),
  ...
  const statCards = [
    { label: t("Total Applications"), value: stats?.total_applications ?? "—", icon: <FaUsers />, color: "blue" },
    { label: t("Active Jobs"), value: stats?.active_jobs ?? "—", icon: <FaBriefcase />, color: "violet" },
    { label: t("This Week"), value: stats?.recent_applications_7days ?? "—", icon: <FaChartLine />, color: "green" },
  ];
    ...
    <div className="hr-module">
      ...
      <motion.div className="hr-page-header" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <div className="d-flex align-items-center">
          ...
          <div>
            <h4>{t("HR & Recruiting Dashboard")}</h4>
            <p>{t("Overview of your hiring pipeline and team activity")}</p>
          </div>
        </div>
      </motion.div>
      ...
      <Row className="g-3 px-1">
        ...
        <Col lg={4}>
          <motion.div className="hr-glass-card" variants={fadeUp} initial="hidden" animate="visible" custom={5}>
            <h6>{t("Applications by Status")}</h6>
            {totalByStatus > 0 ? (
              ...
              <div className="hr-empty-state"><p>{t("No data available")}</p></div>
            )}
          </motion.div>
        </Col>
        ...
        <Col lg={8}>
          <motion.div className="hr-glass-card" variants={fadeUp} initial="hidden" animate="visible" custom={6}>
            <div className="d-flex justify-content-between align-items-center mb-3">
              <h6 className="mb-0">{t("Recent Applications")}</h6>
              <motion.button
            ...
            {recentApps.length > 0 ? (
              ...
              <div className="hr-empty-state"><FaUsers size={36} /><p>{t("No recent applications")}</p></div>
            )}
          </motion.div>
        </Col>
      </Row>
