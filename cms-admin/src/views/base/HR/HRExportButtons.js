...
const HRExportButtons = ({
  ...
  const exportExcel = async () => {
    if (!safeRows.length) {
      notifyError(t("Nothing to export"));
      return;
    }
    setBusy(true);
    try {
      ...
      notifySuccess(t("Excel downloaded"));
    } catch (err) {
      notifyError(t("Failed to export Excel"));
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      notifyError(t("Failed to export PDF"));
      // eslint-disable-next-line no-console
      console.error(err);
    } finally {
  ...
  return (
    <div className="d-inline-flex align-items-center gap-2">
      <motion.button
        ...
        whileTap={{ scale: 0.97 }}
        title={t("Export visible rows as Excel")}
      >
        <FaFileExcel size={12} /> {t("Excel")}
      </motion.button>
      <motion.button
        type="button"
        className="hr-pill-button"
        ...
        whileHover={{ scale: busy ? 1 : 1.03 }}
        whileTap={{ scale: 0.97 }}
        title={t("Export visible rows as PDF")}
      >
        <FaFilePdf size={12} /> {t("PDF")}
      </motion.button>
    </div>
  );
};
