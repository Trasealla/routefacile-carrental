...
const JobShareButtons = ({ jobId, title, publicUrl, compact = false }) => {
  ...
  const channels = [
    ...
    },
    {
      ...
      icon: <SiIndeed />,
      manual: true,
      hint: t("Indeed has no share URL — copy link & paste into your Indeed job posting / company page."),
    },
    {
      key: "instagram",
      label: "Instagram",
      ...
      icon: <FaInstagram />,
      manual: true,
      hint: t("Instagram doesn't allow web sharing — copy link & paste into your bio, story or DM."),
    },
  ];
  ...
  const copyLink = async () => {
    try {
      ...
      notifyError(t("Failed to copy link"));
    }
  };
      ...
      {!compact && (
        <div className="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
          <h6 style={{ marginBottom: 0, display: "flex", alignItems: "center", gap: 8, fontWeight: 700 }}>
            <FaShareAlt size={14} style={{ color: "#228be6" }} /> {t("Share this job")}
          </h6>
          <button
            type="button"
          ...
          >
            <FaShareAlt size={11} /> {t("Quick share")}
          </button>
        </div>
      )}
      ...
      <div className="d-flex flex-wrap gap-2">
        {channels.map((ch) => (
          <a
            ...
            title={ch.hint || t("Share on {{label}}", { label: ch.label })}
            style={{
              display: "inline-flex",
              alignItems: "center",
              gap: 6,
