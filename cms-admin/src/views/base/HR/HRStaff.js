...
const HRStaff = () => {
  ...
  const HR_TYPES = useMemo(
    () => [
      { value: "hr_manager", label: t("HR Manager") },
      { value: "hr_recruitment", label: t("HR Recruitment") },
    ],
    [t]
  );
  ...
  const loadItems = () => {
    ...
    simpleGetCallAuth(`${configWeb.GET_HR_STAFF_LIST}?${params.toString()}`)
      ...
      .catch(() => {
        notifyError(t("Failed to load HR staff"));
        setItems([]);
        setTotal(0);
      })
  ...
  const validate = () => {
    if (!form.first_name.trim() || !form.last_name.trim()) return t("First and last name are required");
    if (!editId && !form.email.trim()) return t("Email is required");
    if (form.email && !/^\S+@\S+\.\S+$/.test(form.email)) return t("Email looks invalid");
    if (!form.phone_number.trim()) return t("Phone number is required");
    if (!HR_TYPES.find((item) => item.value === form.type)) return t("Pick an HR role");
    return null;
  };
  ...
  const onSave = () => {
    ...
    call(url, JSON.stringify(payload))
      .then((res) => {
        if (!res?.error) {
          notifySuccess(editId ? t("Staff updated") : t("Staff created — temp password emailed"));
          formHandlers.close();
          loadItems();
        } else {
          notifyError(
            Array.isArray(res?.message) ? res.message[0] : res?.message || t("Failed to save")
          );
        }
      })
      .catch(() => notifyError(t("Failed to save")))
  ...
  const onToggleStatus = (row) => {
    ...
    simplePatchCallAuth(
      ...
      .then((res) => {
        if (!res?.error) {
          notifySuccess(newStatus === 1 ? t("Staff enabled") : t("Staff disabled"));
          loadItems();
        } else {
          notifyError(
      ...
      .catch(() => notifyError(t("Failed to update status")));
  };
  ...
  const onResetPassword = () => {
    ...
    simplePostCallAuth(
      ...
      .then((res) => {
        if (!res?.error) {
          notifySuccess(t("Temporary password sent to email"));
          resetHandlers.close();
          setResetTarget(null);
        } else {
          notifyError(res?.message || t("Failed to reset password"));
        }
      })
      .catch(() => notifyError(t("Failed to reset password")))
  ...
  const onDelete = () => {
    ...
    simpleDeleteCallAuth(configWeb.DELETE_HR_STAFF(deleteId))
      .then((res) => {
        if (!res?.error) {
          notifySuccess(t("Staff deleted"));
          deleteHandlers.close();
          setDeleteId(null);
          loadItems();
        } else {
          notifyError(res?.message || t("Failed to delete"));
        }
      })
      .catch(() => notifyError(t("Failed to delete")))
      .finally(() => setDeleting(false));
  };

  const statCards = [
    { label: t("Total Staff"), value: stats.total, className: "blue", icon: <FaUsers />, filter: () => { setTypeFilter(""); setStatusFilter(""); } },
    { label: t("Active"), value: stats.active, className: "green", icon: <FaUserCheck />, filter: () => setStatusFilter("1") },
    { label: t("HR Managers"), value: stats.managers, className: "violet", icon: <FaUserShield />, filter: () => setTypeFilter("hr_manager") },
    { label: t("Recruiters"), value: stats.recruiters, className: "orange", icon: <FaUserFriends />, filter: () => setTypeFilter("hr_recruitment") },
  ];
  ...
  return (
    <div className="hr-module">
        ...
        <Row className="align-items-center g-3">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4>{t("HR Staff")}</h4>
                <p>{t("Manage HR managers and recruiters who access this portal")}</p>
              </div>
            </div>
          </Col>
        ...
        {statCards.map((c, idx) => (
          <Col xl={3} lg={3} md={6} sm={6} key={c.label}>
            ...
            >
              ...
              <div className="stat-change up">{t("Click to filter")}</div>
            </motion.div>
          </Col>
        ))}
      </Row>
        ...
        <Row className="align-items-end g-3">
          <Col lg={4} md={6}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.78rem", color: "#64748b" }}>
              <FaSearch size={10} className="me-1" /> {t("Search")}
            </label>
            <Form.Control
              type="text"
          ...
          <Col lg={3} md={4}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.78rem", color: "#64748b" }}>
              <FaFilter size={10} className="me-1" /> {t("Role")}
            </label>
            <Form.Select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} style={{ borderRadius: 10 }}>
              <option value="">{t("All roles")}</option>
              {HR_TYPES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
            </Form.Select>
          </Col>
          <Col lg={3} md={4}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.78rem", color: "#64748b" }}>
          ...
          <table className="hr-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8}><div className="hr-empty-state"><FaUserTie size={34} /><p>{t("No HR staff found")}</p></div></td></tr>
              ) : (
                filtered.map((row) => (
                  <tr key={row.id}>
                    <td>#{row.id}</td>
                    ...
                    <td>
                      <span className="hr-badge" style={{
                        ...
                        {HR_TYPES.find((item) => item.value === row.type)?.label || row.type || "-"}
                      </span>
                    </td>
                    <td>
                      <span className="hr-badge" style={{
                        ...
                        {Number(row.status) === 1 ? t("Active") : t("Disabled")}
                      </span>
                    </td>
                    ...
                    <td style={{ textAlign: "center" }}>
                      <div className="d-flex justify-content-center gap-2">
                        <button
                          ...
                          title={Number(row.status) === 1 ? t("Disable") : t("Enable")}
                        >
                        ...
                        <button className="hr-action-btn edit" onClick={() => onEdit(row)} title={t("Edit")}><FaEdit /></button>
                        <button
        ...
        {!loading && total > 0 && (
          <div className="d-flex justify-content-between align-items-center px-3 py-3">
            <span style={{ fontSize: "0.82rem", color: "#94a3b8" }}>
              {t("Page {{page}} — showing {{count}} of {{total}}", { page, count: filtered.length, total })}
            </span>
            <CustomPagination
      ...
      {/* CREATE / EDIT */}
      <Modal opened={openForm} onClose={formHandlers.close} title={editId ? t("Edit HR Staff") : t("Add HR Staff")} centered radius="lg" size="lg">
        <Row className="g-3">
          <Col md={6}>
          ...
          <Col md={12}>
            <Form.Label>{t("Email")} {editId ? "" : "*"}</Form.Label>
            <Form.Control
              type="email"
              value={form.email}
            ...
            />
            {editId && (
              <small className="text-muted" style={{ fontSize: "0.72rem" }}>{t("Email cannot be changed.")}</small>
            )}
          </Col>
          <Col md={4}>
        ...
        {!editId && (
          <div style={{
            ...
            {t("A temporary password will be generated and emailed to this user.")}
          </div>
        )}
      ...
      <Modal opened={openReset} onClose={resetHandlers.close} title={t("Reset Password")} centered size="sm" radius="lg">
        <p style={{ color: "#475569", fontSize: "0.9rem" }}>
          {t("Send a new temporary password to {{email}}?", { email: resetTarget?.email })}
        </p>
        <div className="d-flex justify-content-end gap-2 mt-3">
          <button className="btn btn-sm" onClick={resetHandlers.close}>{t("Cancel")}</button>
          <button className="btn btn-sm btn-primary" onClick={onResetPassword} disabled={resetting}>
            {resetting ? <Spinner size="sm" /> : t("Send")}
          </button>
        </div>
      </Modal>

      {/* DELETE CONFIRM */}
      <Modal opened={openDelete} onClose={deleteHandlers.close} title={t("Delete HR Staff")} centered size="sm" radius="lg">
        <p style={{ color: "#475569", fontSize: "0.9rem" }}>
          {t("This will permanently remove the HR account. Continue?")}
        </p>
        <div className="d-flex justify-content-end gap-2 mt-3">
