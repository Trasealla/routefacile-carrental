...
const HRApplicationDetail = () => {
  ...
  const fetchAll = () => {
    ...
    Promise.all([
      ...
      .catch(() => notifyError(t("Failed to load application")))
    ...
    Promise.all(promises)
      .then(([updateRes]) => {
        if (updateRes?.status === "success" || (!updateRes?.error)) {
          notifySuccess(t("Application updated"));
          setStatusNotes("");
          fetchAll();
        } else {
          notifyError(Array.isArray(updateRes?.message) ? updateRes.message[0] : updateRes?.message || t("Update failed"));
        }
      })
  ...
  const handleDownloadCV = () => {
    ...
    fetch(configWeb.GET_CAREER_APPLICATION_CV(id), {
      ...
      .catch(() => notifyError(t("Failed to download CV")));
  };
  ...
  const handleDownloadAttachment = (attachmentId, originalName) => {
    ...
    fetch(configWeb.GET_CAREER_APPLICATION_ATTACHMENT(id, attachmentId), {
      ...
      .catch(() => notifyError(t("Failed to download attachment")));
  };

  const handleAddRating = () => {
    if (!ratingValue) return notifyError(t("Please select a rating"));
    ...
    simplePostCallAuth(
      ...
      .then((res) => {
        if (res && !res.error) {
          notifySuccess(t("Rating added"));
          setRatingValue(0);
          setRatingComments("");
          closeRating();
          fetchAll();
        } else {
          notifyError(t("Failed to add rating"));
        }
      })
      .catch(() => notifyError(t("Something went wrong")))
  ...
  const handleSaveAiRating = () => {
    ...
    simplePostCallAuth(
      ...
      .then((res) => {
        if (res && !res.error) {
          notifySuccess(t("AI rating saved"));
          fetchAll();
        } else {
          notifyError(t("Failed to save AI rating"));
        }
      })
      .catch(() => notifyError(t("Something went wrong")))
  ...
  const handleBookInterview = () => {
    if (!bookForm.interview_date) {
      return notifyError(t("Pick a date and time"));
    }
    setBookSaving(true);
    ...
    simplePostCallAuth(
      ...
      .then((res) => {
        if (res && !res.error) {
          notifySuccess(t("Interview booked"));
          setBookForm({ interview_date: "", interview_type: "in-person", location: "", notes: "" });
          closeBook();
          fetchAll();
        } else {
          notifyError(Array.isArray(res?.message) ? res.message[0] : res?.message || t("Failed to book"));
        }
      })
      .catch(() => notifyError(t("Something went wrong")))
    ...
    return (
      <div className="hr-module">
        <div className="hr-empty-state">
          ...
          <Link to="/hr/applications"><button className="btn btn-primary btn-sm">{t('Back to Applications')}</button></Link>
        </div>
      </div>
    );
  }
  ...
  const infoItems = [
    { icon: <FaUser />, label: t('Full Name'), value: getApplicantName(app) },
    { icon: <FaEnvelope />, label: t('Email'), value: app.email },
    { icon: <FaPhone />, label: t('Phone'), value: getApplicantPhone(app) },
    { icon: <FaCalendarAlt />, label: t('Applied Date'), value: formatDate(app.created_at) },
    { icon: <FaBriefcase />, label: t('Job Title'), value: app.career_job?.title_en },
  ].filter((i) => i.value && i.value !== "—");

  const nextAction =
    Number(app.status) <= 0 ? t('Start review') :
    Number(app.status) === 1 ? t('Shortlist or reject') :
    Number(app.status) === 2 ? t('Schedule interview') :
    Number(app.status) === 3 ? t('Collect final decision') :
    Number(app.status) === 5 ? t('Prepare onboarding') : t('Closed outcome');
  ...
  return (
    <div className="hr-module">
          ...
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                ...
                <p style={{ marginBottom: 4 }}>{t('Applied for {{title}}', { title: app.career_job?.title_en || t('N/A') })}</p>
          ...
          <Col xs="auto">
            <div className="d-flex gap-2 flex-wrap">
              <Link to="/hr/interviews">
                ...
                >
                  <FaCalendarAlt size={12} /> {t('Interview Queue')}
                </motion.button>
              </Link>
      ...
      <Row className="g-3 px-1">
        ...
        <Col lg={8}>
          ...
          <motion.div className="hr-detail-card" variants={fadeUp} initial="hidden" animate="visible" custom={0}>
            <h6><FaUser size={13} className="me-2" style={{ color: "#228be6" }} />{t('Applicant Information')}</h6>
            <Row className="g-3">
            ...
            <div className="mt-4 d-flex flex-wrap gap-2">
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
              >
                <FaFileDownload size={14} /> {t('Download CV')}
              </motion.button>
          ...
          <motion.div className="hr-detail-card" variants={fadeUp} initial="hidden" animate="visible" custom={2}>
            <Tabs defaultValue="timeline" variant="pills" radius="lg">
              <Tabs.List mb="lg">
                <Tabs.Tab value="timeline" leftSection={<FaHistory size={12} />}>{t('Status Timeline')}</Tabs.Tab>
                <Tabs.Tab value="interviews" leftSection={<FaCalendarAlt size={12} />}>{t('Interviews ({{count}})', { count: interviews.length })}</Tabs.Tab>
                <Tabs.Tab value="ratings" leftSection={<FaStar size={12} />}>{t('Ratings ({{count}})', { count: ratings.length })}</Tabs.Tab>
              </Tabs.List>
              ...
              <Tabs.Panel value="timeline">
                {statusHistory.length > 0 ? (
                        ...
                        <div className="hr-timeline-item" key={h.id}>
                          ...
                          {h.changed_by_admin && (
                            <div className="hr-timeline-admin">
                              {t('by {{firstName}} {{lastName}}', { firstName: h.changed_by_admin.first_name, lastName: h.changed_by_admin.last_name })}
                            </div>
                          )}
                        </div>
                      );
                  ...
                  <div className="hr-empty-state"><FaHistory size={32} /><p>{t('No status changes yet')}</p></div>
                )}
              </Tabs.Panel>
              ...
              <Tabs.Panel value="interviews">
                {interviews.length > 0 ? (
                  ...
                  <div className="hr-empty-state"><FaCalendarAlt size={32} /><p>{t('No interviews scheduled')}</p></div>
                )}
              </Tabs.Panel>
              ...
              <Tabs.Panel value="ratings">
                ...
                {avgRating && (
                  <div className="d-flex align-items-center gap-3 mb-3 p-3" style={{ background: "#fffbeb", borderRadius: 12, border: "1px solid #fde68a" }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: "0.75rem", color: "#92400e" }}>{t('{{count}} rating(s)', { count: avgRating.total })}</div>
                    </div>
                ...
                {!avgRating && (
                  <div className="mb-3">
                    <motion.button
                      ...
                      whileHover={{ scale: 1.04 }}
                    >
                      <FaPlus size={10} /> {t('Add First Rating')}
                    </motion.button>
                  </div>
                )}
                ...
                {ratings.length > 0 ? (
                  ...
                  <div className="hr-empty-state"><FaStar size={32} /><p>{t('No ratings yet')}</p></div>
                )}
              </Tabs.Panel>
            </Tabs>
          </motion.div>
        ...
        <Col lg={4}>
          <motion.div className="hr-detail-card" variants={fadeUp} initial="hidden" animate="visible" custom={1} style={{ position: "sticky", top: 20 }}>
            <h6><FaClipboardCheck size={13} className="me-2" style={{ color: "#f59e0b" }} />{t('Review & Update')}</h6>
            ...
            >
              ...
              {!screening && (
                <div style={{ fontSize: "0.8rem", color: "#64748b" }}>
                  {t('No keywords configured for this job. Add some in')} <Link to="/hr/keywords" style={{ color: "#1d4ed8", fontWeight: 600 }}>{t('AI Keywords')}</Link> {t('to enable auto screening.')}
                </div>
              )}
                ...
                <>
                  <div style={{ fontSize: "0.7rem", color: "#475569", marginBottom: 4 }}>
                    {cvParsing
                      ...
                      : screening.cvParsed
                        ? t("CV scanned ✓")
                        : t("CV not readable — using application fields only")}
                  </div>
                  <div className="d-flex align-items-baseline gap-2" style={{ marginBottom: 8 }}>
                    ...
                    <div style={{ fontSize: "0.72rem", color: "#475569" }}>{t('match score')}</div>
                  </div>
                  ...
                  {screening.mustHave.length > 0 && (
                    <div style={{ marginTop: 10 }}>
                      <div style={{ fontSize: "0.7rem", fontWeight: 700, color: "#475569", marginBottom: 4 }}>{t('MUST HAVE')}</div>
                  ...
                  {screening.optional.length > 0 && (
                    <div style={{ marginTop: 8 }}>
                      <div style={{ fontSize: "0.7rem", fontWeight: 700, color: "#475569", marginBottom: 4 }}>{t('OPTIONAL')}</div>
                      <div className="d-flex flex-wrap gap-1">
                  ...
                  >
                    {savingAiRating ? <Spinner size="sm" animation="border" /> : t('Save as AI Rating')}
                  </motion.button>
                </>
              )}
            </div>
            ...
            <div className="hr-spotlight-card mb-3">
              <div style={{ fontSize: "0.72rem", fontWeight: 700, color: "#1d4ed8", textTransform: "uppercase", letterSpacing: "0.6px" }}>{t('Quick snapshot')}</div>
              <Row className="g-2 mt-1">
                ...
                <Col xs={6}>
                  <div className="hr-mini-metric" style={{ padding: 12 }}>
                    <div className="label">{t('Next step')}</div>
            ...
            <Form.Group className="mb-3">
              <Form.Label style={{ fontSize: "0.78rem", fontWeight: 600, color: "#64748b" }}>{t('Application Status')}</Form.Label>
            ...
            {Number(status) !== app?.status && (
              <Form.Group className="mb-3">
                <Form.Label style={{ fontSize: "0.78rem", fontWeight: 600, color: "#64748b" }}>{t('Status Change Notes')}</Form.Label>
                <Form.Control
                  as="textarea"
                  rows={2}
                  value={statusNotes}
                  onChange={(e) => setStatusNotes(e.target.value)}
                  placeholder={t('Reason for status change...')}
            ...
            <Form.Group className="mb-4">
              <Form.Label style={{ fontSize: "0.78rem", fontWeight: 600, color: "#64748b" }}>{t('Admin Notes')}</Form.Label>
              <Form.Control
                as="textarea"
                rows={4}
                value={adminNotes}
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
            >
              {updateLoading ? <Spinner size="sm" /> : t('Update Application')}
            </motion.button>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
            >
              <FaCalendarAlt size={12} /> {t('Book Interview')}
            </motion.button>
          </motion.div>
        </Col>
      </Row>
      ...
      <Modal opened={bookOpen} onClose={closeBook} title={t('Book Interview')} centered radius="lg" size="md">
        <Form.Group className="mb-3">
          <Form.Label style={{ fontSize: "0.82rem", fontWeight: 600, color: "#475569" }}>{t('Date & Time')}</Form.Label>
          <Form.Control
            type="datetime-local"
        ...
        <Row className="g-2">
          <Col md={6}>
            <Form.Group className="mb-3">
              <Form.Label style={{ fontSize: "0.82rem", fontWeight: 600, color: "#475569" }}>{t('Type')}</Form.Label>
              <Form.Select
          ...
          <Col md={6}>
            <Form.Group className="mb-3">
              <Form.Label style={{ fontSize: "0.82rem", fontWeight: 600, color: "#475569" }}>{t('Location / Link')}</Form.Label>
              <Form.Control
                type="text"
                value={bookForm.location}
                onChange={(e) => setBookForm({ ...bookForm, location: e.target.value })}
                placeholder={t('Office, Zoom, etc.')}
                style={{ borderRadius: 10 }}
              />
            </Form.Group>
          </Col>
        ...
        <Form.Group className="mb-3">
          <Form.Label style={{ fontSize: "0.82rem", fontWeight: 600, color: "#475569" }}>{t('Notes')}</Form.Label>
          <Form.Control
            as="textarea"
            rows={3}
            value={bookForm.notes}
            onChange={(e) => setBookForm({ ...bookForm, notes: e.target.value })}
            placeholder={t('Agenda, panelists, etc.')}
            style={{ borderRadius: 10, resize: "vertical" }}
          />
        </Form.Group>
        <div className="d-flex justify-content-end gap-2">
          <button className="btn btn-light" style={{ borderRadius: 10 }} onClick={closeBook}>{t('Cancel')}</button>
          <motion.button
            className="btn"
            ...
            whileHover={{ scale: 1.02 }}
          >
            {bookSaving ? <Spinner size="sm" animation="border" /> : t('Confirm Booking')}
          </motion.button>
        </div>
      </Modal>

      {/* Add Rating Modal */}
      <Modal opened={ratingOpen} onClose={closeRating} title={t('Add Rating')} centered radius="lg" size="sm">
        <div className="mb-3 text-center">
          <Rating value={ratingValue} onChange={setRatingValue} size="xl" />
        </div>
        <Form.Group className="mb-3">
          <Form.Label style={{ fontSize: "0.82rem", fontWeight: 600, color: "#475569" }}>{t('Comments')}</Form.Label>
          <Form.Control
            as="textarea"
            rows={3}
        ...
        <motion.button
          ...
          whileHover={{ scale: 1.02 }}
        >
          {ratingSaving ? <Spinner size="sm" /> : t('Submit Rating')}
        </motion.button>
      </Modal>
    </div>
  );
