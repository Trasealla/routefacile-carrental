...
const HRJobForm = () => {
      ...
      simpleGetCallAuth(configWeb.GET_CAREER_JOB_DETAILS(id))
        .then((res) => {
          if (res && !res.error) {
            ...
            notifyError(t("Failed to load job details"));
          }
        })
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.title_en || !formData.description_en || !formData.location_en || !formData.expiry_date) {
      return notifyError(t("Please fill in all required English fields"));
    }
    setSaving(true);
    ...
    const body = JSON.stringify({
      ...
      title_ar: (formData.title_ar || "").trim() || formData.title_en,
      description_en: formData.description_en,
      description_ar: (formData.description_ar || "").trim() || formData.description_en,
      location_en: formData.location_en,
      location_ar: (formData.location_ar || "").trim() || formData.location_en,
        ...
        if (res && !res.error) {
          ...
          if (!isEdit && newJobId && stagedKeywords.length) {
            try {
              ...
              notifyError(t("Job saved, but some keywords could not be created"));
            }
          }
          notifySuccess(isEdit ? t("Job updated successfully") : t("Job created successfully"));
          if (isEdit) {
            navigate("/hr/jobs");
          } else {
    ...
    <div className="hr-module">
        ...
        <Row className="align-items-center">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4>{isEdit ? t("Edit Job Posting") : t("Create New Job")}</h4>
                <p>{isEdit ? t("Update the job details below") : t("Fill in the details to publish a new job")}</p>
              </div>
            </div>
          </Col>
          <Col xs="auto">
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
            >
              <FaArrowLeft size={12} /> {t("Back to Jobs")}
            </motion.button>
          </Col>
        </Row>
      </motion.div>
            ...
            <motion.div className="hr-detail-card" variants={fadeUp} initial="hidden" animate="visible" custom={0}>
              <Tabs defaultValue="english" variant="pills" radius="lg">
                <Tabs.List mb="lg">
                  ...
                  <Tabs.Tab value="arabic" leftSection={<FaGlobe size={12} />}>{t("Arabic")} <span style={{ fontSize: "0.7rem", color: "#94a3b8", marginLeft: 4 }}>{t("(optional)")}</span></Tabs.Tab>
                </Tabs.List>

                <Tabs.Panel value="english">
                  <Form.Group className="mb-3">
                    <Form.Label className="fw-semibold" style={{ fontSize: "0.82rem", color: "#475569" }}>{t("Job Title (EN) *")}</Form.Label>
                    <Form.Control
                  ...
                  <Form.Group className="mb-3">
                    <Form.Label className="fw-semibold" style={{ fontSize: "0.82rem", color: "#475569" }}>{t("Location (EN) *")}</Form.Label>
                    <Form.Control
                ...
                <Tabs.Panel value="arabic">
                  <div style={{
                    ...
                    {t("Arabic fields are optional. Leave any blank and the English value will be used automatically.")}
                  </div>
                  <Form.Group className="mb-3">
                    <Form.Label className="fw-semibold" style={{ fontSize: "0.82rem", color: "#475569" }}>{t("Job Title (AR)")} <span style={{ color: "#94a3b8", fontWeight: 500 }}>{t("— optional")}</span></Form.Label>
                    <Form.Control
                  ...
                  <Form.Group className="mb-3">
                    <Form.Label className="fw-semibold" style={{ fontSize: "0.82rem", color: "#475569" }}>{t("Description (AR)")} <span style={{ color: "#94a3b8", fontWeight: 500 }}>{t("— optional")}</span></Form.Label>
                    <CKEditorComponent
                      contentW={formData.description_ar}
                      onContentChange={(val) => handleChange("description_ar", val)}
                    />
                  </Form.Group>
                  <Form.Group className="mb-3">
                    <Form.Label className="fw-semibold" style={{ fontSize: "0.82rem", color: "#475569" }}>{t("Location (AR)")} <span style={{ color: "#94a3b8", fontWeight: 500 }}>{t("— optional")}</span></Form.Label>
                    <Form.Control
          ...
          <Col lg={4}>
            <motion.div className="hr-detail-card" variants={fadeUp} initial="hidden" animate="visible" custom={1}>
              <h6 style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <FaBriefcase size={14} style={{ color: "#228be6" }} /> {t("Job Settings")}
              </h6>

              <Form.Group className="mb-3">
                <Form.Label className="fw-semibold" style={{ fontSize: "0.82rem", color: "#475569" }}>{t("Experience Years")}</Form.Label>
                <Form.Control
                  type="number"
                  min="0"
              ...
              <Form.Group className="mb-3">
                <Form.Label className="fw-semibold" style={{ fontSize: "0.82rem", color: "#475569" }}>{t("Expiry Date *")}</Form.Label>
                <Form.Control
                  type="date"
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
              >
                {saving ? <Spinner size="sm" /> : <><FaSave size={14} /> {isEdit ? t("Update Job") : t("Publish Job")}</>}
              </motion.button>
            </motion.div>
          </Col>
      ...
      <Modal
        ...
        title={
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontWeight: 700 }}>{t("Job published — share it now")}</span>
          </div>
        }
        centered
        size="lg"
      ...
      >
        <p style={{ color: "#475569", fontSize: "0.88rem", marginBottom: 16 }}>
          {t("Your job posting is live. Spread the word across your channels to attract more candidates.")}
        </p>
