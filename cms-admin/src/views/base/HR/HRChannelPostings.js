...
const CHANNEL_NAMES = ["routefacile", "indeed", "linkedin", "naukrigulf", "facebook", "instagram", "tiktok"];
const POSTING_STATUS = ["queued", "posted", "failed", "retrying"];

const INITIAL_FORM = {
  career_job_id: "",
  channel_name: "routefacile",
  external_post_id: "",
...
const HRChannelPostings = () => {
  ...
  const loadItems = () => {
    ...
    simpleGetCallAuth(`${configWeb.GET_RECRUITING_CHANNEL_POSTING_LIST}?${params.toString()}`)
      ...
      .catch(() => {
        notifyError(t("Failed to load channel postings"));
        setItems([]);
      })
  ...
  const onCreateClick = () => {
    setEditId(null);
    setForm({ ...INITIAL_FORM, career_job_id: selectedJob || "", channel_name: selectedChannel || "routefacile" });
    formHandlers.open();
  };
  ...
  const onEditClick = (row) => {
    ...
    setForm({
      ...
      channel_name: row.channel_name || "routefacile",
  ...
  const onSave = () => {
    if (!form.career_job_id || !form.channel_name || !form.posting_status) {
      notifyError(t("Job, channel and posting status are required"));
      return;
    }
    ...
    if (!editId) {
      ...
      simplePostCallAuth(configWeb.POST_RECRUITING_CHANNEL_POSTING_CREATE, JSON.stringify(payload))
        .then((res) => {
          if (!res?.error) {
            notifySuccess(t("Channel record created"));
            formHandlers.close();
            loadItems();
          } else {
            notifyError(res?.message || t("Failed to create channel record"));
          }
        })
        .catch(() => notifyError(t("Failed to create channel record")))
    ...
    simplePutCallAuth(configWeb.PUT_RECRUITING_CHANNEL_POSTING_UPDATE(editId), JSON.stringify(payload))
      .then((res) => {
        if (!res?.error) {
          notifySuccess(t("Channel record updated"));
          formHandlers.close();
          loadItems();
        } else {
          notifyError(res?.message || t("Failed to update channel record"));
        }
      })
      .catch(() => notifyError(t("Failed to update channel record")))
      .finally(() => setSaving(false));
  };
  ...
  const onQuickStatusChange = (row, nextStatus) => {
    simplePutCallAuth(
      ...
      .then((res) => {
        if (!res?.error) {
          notifySuccess(t("Posting status updated"));
          loadItems();
        } else {
          notifyError(res?.message || t("Failed to update posting status"));
        }
      })
      .catch(() => notifyError(t("Failed to update posting status")));
  };
  ...
  const onDelete = () => {
    ...
    simpleDeleteCallAuth(configWeb.DELETE_RECRUITING_CHANNEL_POSTING(deleteId))
      .then((res) => {
        if (!res?.error) {
          notifySuccess(t("Channel record deleted"));
          deleteHandlers.close();
          setDeleteId(null);
          loadItems();
        } else {
          notifyError(res?.message || t("Failed to delete channel record"));
        }
      })
      .catch(() => notifyError(t("Failed to delete channel record")))
    ...
    <div className="hr-module">
        ...
        <Row className="align-items-center g-3">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4>{t("Channel Postings")}</h4>
                <p>{t("Track publishing state across recruitment channels")}</p>
              </div>
            </div>
          </Col>
          <Col xs="auto" className="d-flex align-items-center gap-2 flex-wrap">
            <HRExportButtons
              ...
              columns={[
                ...
                { header: t("Channel"), accessor: (r) => r.channel_name || "" },
                { header: t("Status"), accessor: (r) => r.posting_status || "" },
                { header: t("Status Message"), accessor: (r) => r.status_message || "" },
                { header: t("Posted At"), accessor: (r) => formatDateTime(r.posted_at) },
                { header: t("Created"), accessor: (r) => formatDateTime(r.created_at) },
                { header: t("Active"), accessor: (r) => (r.status === 1 ? t("Yes") : t("No")) },
              ]}
            />
            <button className="hr-pill-button primary" onClick={onCreateClick}>
              <FaPlus size={12} /> {t("Add Channel Record")}
            </button>
          </Col>
        </Row>
      </div>
      ...
      <div className="hr-filter-bar mx-1 mt-3">
        <Row className="align-items-end g-3">
          ...
          <Col lg={2} md={6}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.78rem", color: "#64748b" }}>
              <FaFilter size={10} className="me-1" /> {t("Channel")}
            </label>
          ...
          <Col lg={4} md={8}>
            <div style={{ position: "relative" }}>
              ...
              <Form.Control
                type="text"
                placeholder={t("Search by channel, status, post ID...")}
                value={searchTerm}
          ...
          <table className="hr-table">
            <thead>
              <tr>
                ...
                <th>{t("Job")}</th>
                <th>{t("Channel")}</th>
                <th>{t("Posting Status")}</th>
                <th>{t("External Post ID")}</th>
                <th>{t("Last Synced")}</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8}><div className="hr-empty-state"><FaBroadcastTower size={34} /><p>{t("No channel postings found")}</p></div></td></tr>
              ) : (
                paginated.map((row) => (
                  <tr key={row.id}>
                    <td>#{row.id}</td>
                    ...
                    <td style={{ textAlign: "center" }}>
                      <div className="d-flex justify-content-center gap-2 flex-wrap">
                        ...
                        {row.posting_status === "queued" && (
                          <button className="btn btn-sm btn-outline-primary" onClick={() => onQuickStatusChange(row, "posted")}>{t("Mark Posted")}</button>
                        )}
                        {row.posting_status === "posted" && (
                          <button className="btn btn-sm btn-outline-danger" onClick={() => onQuickStatusChange(row, "failed")}>{t("Mark Failed")}</button>
                        )}
                        {row.posting_status === "failed" && (
                          <button className="btn btn-sm btn-outline-warning" onClick={() => onQuickStatusChange(row, "retrying")}>{t("Retry")}</button>
                        )}
                      </div>
                    </td>
                  </tr>
      ...
      <Modal opened={openForm} onClose={formHandlers.close} title={editId ? t("Edit Channel Posting") : t("Create Channel Posting")} centered radius="lg" size="lg">
        <Row className="g-3">
          <Col md={6}>
          ...
          <Col md={6}>
            <Form.Label>{t("Posting Status")} *</Form.Label>
      ...
      <Modal opened={openDelete} onClose={deleteHandlers.close} title={t("Delete Channel Posting")} centered size="sm" radius="lg">
        <p style={{ color: "#475569", fontSize: "0.9rem" }}>{t("Are you sure you want to delete this channel posting record?")}</p>
        <div className="d-flex justify-content-end gap-2 mt-3">
