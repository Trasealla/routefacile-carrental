...
const HRStaffDashboard = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(() => notifyError(t("Failed to load dashboard")))
  ...
  const getTimeUntil = (dateStr) => {
    ...
    if (diff < 0) return t("Passed");
    const hours = Math.floor(diff / (1000 * 60 * 60));
    const days = Math.floor(hours / 24);
    if (days > 0) return t("in {{days}}d {{hours}}h", { days, hours: hours % 24 });
    if (hours > 0) return t("in {{hours}}h", { hours });
    const mins = Math.floor(diff / (1000 * 60));
    return t("in {{mins}}m", { mins });
  };
  ...
  const modeBreakdown = useMemo(() => {
    ...
    return [
      { label: t("Video"), value: video, color: "#228be6" },
      { label: t("Phone"), value: phone, color: "#16a34a" },
      { label: t("On-site"), value: inPerson, color: "#f97316" },
    ];
  }, [myInterviews]);
  ...
  const statCards = [
    {
      ...
      icon: <FaBriefcase />,
      className: "blue",
      change: t("{{count}} visible now", { count: openPositions.length }),
    },
    {
      ...
      className: "orange",
      change: t("{{count}} awaiting follow-up", { count: Math.max((stats.pending_applications || 0) - myInterviews.length, 0) }),
    },
    {
      label: t("Upcoming"),
      ...
      className: "teal",
      change: todayInterviews ? t("{{count}} today", { count: todayInterviews }) : t("Nothing due today"),
    },
    {
      label: t("Completed"),
      ...
      icon: <FaCheckCircle />,
      className: "green",
      change: t("{{rate}}% completion rate", { rate: completionRate }),
    },
    {
      label: t("Ratings Given"),
      value: stats.my_ratings_given || 0,
      icon: <FaStar />,
      className: "rose",
      change: t("{{avg}}/5 avg", { avg: ratingAverage ? ratingAverage.toFixed(1) : "0.0" }),
    },
  ];
        ...
        <Row className="align-items-center g-3">
          <Col lg={7}>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4>{t("My Recruiting Workspace")}</h4>
                <p>{t("Track interviews, candidate feedback and today’s hiring priorities in one place")}</p>
                <div className="hr-hero-pills">
                  <span className="hr-hero-pill">
                    <FaRegCalendarAlt size={11} /> {t("{{count}} today", { count: todayInterviews })}
                  </span>
                  <span className="hr-hero-pill">
                    <FaStar size={11} /> {t("{{avg}}/5 average", { avg: ratingAverage ? ratingAverage.toFixed(1) : "0.0" })}
                  </span>
                  <span className="hr-hero-pill">
                    <FaTasks size={11} /> {t("{{count}} open roles", { count: openPositions.length })}
                  </span>
                </div>
              </div>
            </div>
          ...
          <Col lg={5}>
            <div className="hr-header-actions">
              <motion.button className="hr-pill-button primary" onClick={() => navigate("/hr/interviews")} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }}>
                <FaCalendarCheck size={12} /> {t("Review interviews")}
              </motion.button>
          ...
          <motion.div className="hr-dash-card" variants={fadeUp} custom={5} initial="hidden" animate="visible">
            <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
              <div className="card-title mb-0">
                <FaChartLine /> {t("My Interview Rhythm")}
              </div>
              <span style={{ fontSize: "0.75rem", color: "#64748b", fontWeight: 700 }}>{t("Next 7 days")}</span>
            </div>

            <Row className="g-3 mb-3">
              <Col md={4}>
                <div className="hr-mini-metric">
                  <div className="label">{t("Next interview")}</div>
                  <div className="value">{nextInterview ? formatDate(nextInterview.interview_date) : t("Clear")}</div>
                  <div className="sub">{nextInterview ? getTimeUntil(nextInterview.interview_date) : t("No upcoming meeting")}</div>
                </div>
              </Col>
              <Col md={4}>
                <div className="hr-mini-metric">
                  <div className="label">{t("Execution rate")}</div>
                  <div className="value">{completionRate}%</div>
                  <div className="sub">{t("Completed vs scheduled interviews")}</div>
                </div>
              </Col>
              <Col md={4}>
                <div className="hr-mini-metric">
                  <div className="label">{t("Feedback quality")}</div>
                  <div className="value">{ratingAverage ? ratingAverage.toFixed(1) : "0.0"}</div>
                  <div className="sub">{t("Average score from submitted ratings")}</div>
                </div>
              </Col>
            </Row>
            ...
            <div style={{ height: 220 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={upcomingTrend} margin={{ top: 10, right: 4, left: -18, bottom: 0 }}>
                  ...
                  <RechartsTooltip
                    formatter={(value) => [t("{{count}} interview(s)", { count: value }), t("Scheduled")]}
      ...
      <Row className="mx-1 mt-3 g-3">
        <Col lg={7}>
          <motion.div className="hr-dash-card" variants={fadeUp} custom={7} initial="hidden" animate="visible">
            <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
              <div className="card-title mb-0">
                <FaRegCalendarAlt /> {t("Upcoming Interviews")}
              </div>
              <motion.button
              ...
              >
                {t("All interviews")} <FaArrowRight size={10} className="ms-1" />
              </motion.button>
            </div>
            ...
            {myInterviews.length === 0 ? (
              <div className="hr-empty-state" style={{ minHeight: 120 }}>
                ...
                <p>{t("No upcoming interviews assigned to you")}</p>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="d-flex align-items-center gap-3" style={{ flex: 1 }}>
                        ...
                        <div>
                          ...
                          <div className="sub d-flex align-items-center gap-1" style={{ textTransform: "capitalize" }}>
                            {interviewTypeIcon(iv.interview_type)} {iv.interview_type || t("in-person")}
                      ...
                      <div style={{ textAlign: "right" }}>
                        ...
                        <div style={{ fontSize: "0.72rem", fontWeight: 700, color: isToday ? "#228be6" : timeLabel === t("Passed") ? "#ef4444" : "#16a34a" }}>
                          {isToday ? t("TODAY · ") : ""}{timeLabel}
                        </div>
                      </div>
                    </motion.div>
                  );
        ...
        <Col lg={5}>
          <motion.div className="hr-dash-card h-100" variants={fadeUp} custom={8} initial="hidden" animate="visible">
            <div className="card-title">
              <FaStar /> {t("Recent Ratings")}
            </div>

            <div className="hr-spotlight-card mb-3">
              <div style={{ fontSize: "0.72rem", fontWeight: 700, color: "#1d4ed8", textTransform: "uppercase", letterSpacing: "0.6px" }}>{t("Feedback score")}</div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginTop: 8 }}>
                <div>
                  <div style={{ fontSize: "1.8rem", fontWeight: 800, color: "#0f172a" }}>{ratingAverage ? ratingAverage.toFixed(1) : "0.0"}</div>
                  <div style={{ fontSize: "0.78rem", color: "#64748b" }}>{t("{{count}} rating(s) submitted", { count: myRatings.length })}</div>
                </div>
            ...
            {myRatings.length === 0 ? (
              <div className="hr-empty-state" style={{ minHeight: 100 }}>
                ...
                <p>{t("No ratings submitted yet")}</p>
              </div>
            ) : (
              <div className="hr-insight-list">
                    ...
                    <div className="d-flex align-items-center gap-2" style={{ flex: 1 }}>
                      ...
                      <div>
                        ...
                        <div className="sub" style={{ maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {r.comments || t("Feedback submitted")}
                        </div>
                      </div>
                    </div>
                    <div className="d-flex align-items-center gap-1">
      ...
      <Row className="mx-1 mt-3 g-3 mb-4">
        <Col lg={6}>
          <motion.div className="hr-dash-card" variants={fadeUp} custom={9} initial="hidden" animate="visible">
            <div className="card-title">
              <FaCheckCircle /> {t("Recently Completed Interviews")}
            </div>
            {recentInterviews.length === 0 ? (
              <div className="hr-empty-state" style={{ minHeight: 100 }}>
                <FaCheckCircle size={24} />
                <p>{t("No completed interviews")}</p>
              </div>
            ) : (
                  ...
                  <tbody>
                    {recentInterviews.map((iv) => (
                      <tr key={iv.id} style={{ cursor: "pointer" }} onClick={() => navigate(`/hr/interview/${iv.id}`)}>
                        ...
                        <td>
                          <Tooltip label={iv.feedback || t("No feedback")} withArrow multiline w={220}>
          ...
          <motion.div className="hr-dash-card" variants={fadeUp} custom={10} initial="hidden" animate="visible">
            <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.04 }}
              >
                {t("View all")} <FaArrowRight size={10} className="ms-1" />
              </motion.button>
            </div>
                    ...
                    >
                      <div>
                        ...
                        <div className="sub">
                          ...
                          {job.experience_years != null ? ` · ${t("{{years}} yrs exp", { years: job.experience_years })}` : ""}
                        </div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        {daysLeft != null && (
                          ...
                          >
                            {daysLeft <= 0 ? t("Expired") : t("{{days}}d left", { days: daysLeft })}
                          </span>
                        )}
                        <div style={{ fontSize: "0.7rem", color: "#94a3b8", marginTop: 4 }}>{t("Expires {{date}}", { date: formatDate(job.expiry_date) })}</div>
                      </div>
                    </motion.div>
                  );
                })}
