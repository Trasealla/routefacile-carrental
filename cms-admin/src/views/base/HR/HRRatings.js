...
const HRRatings = () => {
  ...
  const loadItems = () => {
    ...
    simpleGetCallAuth(`${configWeb.GET_RECRUITING_RATING_LIST}?${params.toString()}`)
      ...
      .catch(() => {
        notifyError(t("Failed to load ratings"));
        setItems([]);
      })
  ...
  const onSave = () => {
    if (!form.application_id || !form.rating) {
      notifyError(t("Application and rating are required"));
      return;
    }
    ...
    if (!editId) {
      ...
      simplePostCallAuth(configWeb.POST_RECRUITING_RATING_CREATE, JSON.stringify(payload))
        .then((res) => {
          if (!res?.error) {
            notifySuccess(t("Rating created"));
            formHandlers.close();
            loadItems();
          } else {
            notifyError(res?.message || t("Failed to create rating"));
          }
        })
        .catch(() => notifyError(t("Failed to create rating")))
    ...
    simplePutCallAuth(configWeb.PUT_RECRUITING_RATING_UPDATE(editId), JSON.stringify(payload))
      .then((res) => {
        if (!res?.error) {
          notifySuccess(t("Rating updated"));
          formHandlers.close();
          loadItems();
        } else {
          notifyError(res?.message || t("Failed to update rating"));
        }
      })
      .catch(() => notifyError(t("Failed to update rating")))
      .finally(() => setSaving(false));
  };
  ...
  const onDelete = () => {
    ...
    simpleDeleteCallAuth(configWeb.DELETE_RECRUITING_RATING(deleteId))
      .then((res) => {
        if (!res?.error) {
          notifySuccess(t("Rating deleted"));
          deleteHandlers.close();
          setDeleteId(null);
          loadItems();
        } else {
          notifyError(res?.message || t("Failed to delete rating"));
        }
      })
      .catch(() => notifyError(t("Failed to delete rating")))
    ...
    <div className="hr-module">
        ...
        <Row className="align-items-center g-3">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4>{t('Ratings')}</h4>
                <p>{t('Manage recruiter ratings and feedback for applications')}</p>
              </div>
            </div>
          </Col>
      ...
      <div className="hr-filter-bar mx-1 mt-3">
        <Row className="align-items-end g-3">
          ...
          <Col lg={2} md={6}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.78rem", color: "#64748b" }}>
              <FaFilter size={10} className="me-1" /> {t('Rating')}
            </label>
          ...
          <Col lg={4} md={8}>
            <div style={{ position: "relative" }}>
              ...
              <Form.Control
                type="text"
                placeholder={t('Search by comments, applicant, ID...')}
                value={searchTerm}
      ...
      {avgData && (
        <div className="mx-1 mt-3 p-3" style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 12 }}>
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              <div style={{ fontWeight: 700, color: "#92400e" }}>{t('Average Rating')}</div>
              <div style={{ color: "#78350f", fontSize: "0.9rem" }}>
                {t('{{avg}} / 5 from {{total}} rating(s)', { avg: avgData.average.toFixed(1), total: avgData.total })}
              </div>
            </div>
          </div>
        </div>
          ...
          <table className="hr-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7}><div className="hr-empty-state"><FaStar size={34} /><p>{t('No ratings found')}</p></div></td></tr>
              ) : (
                paginated.map((row) => (
                  <tr key={row.id}>
                    <td>#{row.id}</td>
                    ...
                    <td style={{ textAlign: "center" }}>
                      <div className="d-flex justify-content-center gap-2">
                        <button className="hr-action-btn edit" onClick={() => openEdit(row)} title={t('Edit')}><FaEdit /></button>
                        <button
      ...
      <Modal opened={openForm} onClose={formHandlers.close} title={editId ? t('Edit Rating') : t('Create Rating')} centered radius="lg" size="lg">
        <Row className="g-3">
          <Col md={12}>
      ...
      <Modal opened={openDelete} onClose={deleteHandlers.close} title={t('Delete Rating')} centered size="sm" radius="lg">
        <p style={{ color: "#475569", fontSize: "0.9rem" }}>{t('Are you sure you want to delete this rating?')}</p>
        <div className="d-flex justify-content-end gap-2 mt-3">
