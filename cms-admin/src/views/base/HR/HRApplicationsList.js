...
const HRApplicationsList = () => {
  ...
  const loadApplications = (status, job) => {
    ...
    simpleGetCallAuth(`${configWeb.GET_CAREER_APPLICATION_LIST}?${params.toString()}`)
      ...
      .catch(() => { notifyError(t("Failed to load applications")); setApplications([]); })
  ...
  const quickStatuses = [
    ...
    { label: t("Reviewing"), value: "1" },
    { label: t("Shortlisted"), value: "2" },
    { label: t("Hired"), value: "5" },
  ];
  ...
  const confirmDelete = () => {
    ...
    simpleDeleteCallAuth(configWeb.DELETE_CAREER_APPLICATION(deleteId))
      .then((res) => {
        if (res && !res.error) { notifySuccess(t("Application deleted")); loadApplications(); }
  ...
  return (
    <div className="hr-module">
        ...
        <Row className="align-items-center g-3">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                ...
                <p>{t("Review candidates, monitor the funnel and move the best talent faster")}</p>
              </div>
            </div>
          </Col>
          <Col xs="auto" className="d-flex align-items-center gap-2 flex-wrap">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
            >
              <FaCalendarCheck size={12} /> {t("Interview Queue")}
            </motion.button>
          </Col>
        </Row>
      </motion.div>
      ...
      <Row className="mb-3 g-3 px-1">
        {[
          { label: t("Total"), value: stats.total, icon: <FaUsers />, color: "blue" },
          { label: t("Pending"), value: stats.pending, icon: <FaClock />, color: "amber" },
          { label: t("Shortlisted"), value: stats.shortlisted, icon: <FaCheckCircle />, color: "cyan" },
          { label: t("Hired"), value: stats.hired, icon: <FaCheckCircle />, color: "green" },
          { label: t("Rejected"), value: stats.rejected, icon: <FaTimesCircle />, color: "red" },
        ].map((s, i) => (
            ...
            <Row className="g-3">
              <Col md={4}>
                <div className="hr-mini-metric">
                  <div className="label">{t("Shortlist rate")}</div>
                  <div className="value">{shortlistRate}%</div>
                  <div className="sub">{t("Candidates that made it to shortlist")}</div>
                </div>
              </Col>
              <Col md={4}>
                <div className="hr-mini-metric">
                  <div className="label">{t("Reviewing now")}</div>
                  <div className="value">{stats.reviewing}</div>
                  <div className="sub">{t("Applications currently under review")}</div>
                </div>
              </Col>
              <Col md={4}>
                <div className="hr-mini-metric">
                  <div className="label">{t("New this week")}</div>
                  <div className="value">{stats.recent}</div>
                  <div className="sub">{t("Fresh applications in the last 7 days")}</div>
                </div>
              </Col>
            </Row>
          </motion.div>
        ...
        <Col lg={4}>
          <motion.div className="hr-dash-card h-100" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22, duration: 0.4 }}>
            <div className="card-title mb-3">
              <FaUsers /> {t("Recent Candidates")}
            </div>
            {recentApplicants.length === 0 ? (
              <div className="hr-empty-state" style={{ minHeight: 100, padding: "24px 10px" }}>
                <FaUsers size={22} />
                <p style={{ marginBottom: 0 }}>{t("No recent applications")}</p>
              </div>
            ) : (
                    ...
                    <div key={candidate.id} className="hr-insight-item" style={{ cursor: "pointer" }} onClick={() => navigate(`/hr/applications/${candidate.id}`)}>
                      <div className="d-flex align-items-center gap-2" style={{ flex: 1 }}>
                        ...
                        <div>
                          ...
                          <div className="sub">{candidate.career_job?.title_en || t("No job linked")}</div>
                        </div>
                      </div>
        ...
        <Row className="align-items-end g-3">
          <Col lg={3} md={6}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.78rem", color: "#64748b" }}>
              <FaFilter size={10} className="me-1" /> {t("Filter by Job")}
            </label>
          ...
          <Col lg={2} md={6}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.78rem", color: "#64748b" }}>
              <FaFilter size={10} className="me-1" /> {t("Source")}
            </label>
          ...
          <Col lg={2} md={6}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.78rem", color: "#64748b" }}>{t("Date From")}</label>
            <Form.Control type="date" value={dateFrom} onChange={(e) => updateParam("date_from", e.target.value)} style={{ borderRadius: 10 }} />
          </Col>
          <Col lg={2} md={6}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.78rem", color: "#64748b" }}>{t("Date To")}</label>
            ...
            <AnimatePresence mode="popLayout">
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={10}><div className="hr-empty-state"><FaUsers size={40} /><p>{t("No applications found")}</p></div></td></tr>
                ) : (
                      ...
                      <motion.tr key={row.id} variants={fadeUp} custom={idx} initial="hidden" animate="visible" exit={{ opacity: 0 }} layout>
                        ...
                        <td style={{ textAlign: "center" }}>
                          <div className="d-flex justify-content-center gap-2">
                            <motion.button className="hr-action-btn view" onClick={() => navigate(`/hr/applications/${row.id}`)} whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.9 }} title={t("View")}>
                              <FaEye />
                            </motion.button>
                            <motion.button className="hr-action-btn delete" onClick={() => { setDeleteId(row.id); openDelete(); }} whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.9 }} title={t("Delete")}>
                              <FaTrash />
                            </motion.button>
                          </div>
                        </td>
      ...
      {/* Delete Modal */}
      <Modal opened={deleteOpen} onClose={closeDelete} title={t("Delete Application")} centered size="sm" radius="lg">
        <p style={{ color: "#475569", fontSize: "0.9rem" }}>{t("Are you sure? This will permanently remove this application and all attachments.")}</p>
