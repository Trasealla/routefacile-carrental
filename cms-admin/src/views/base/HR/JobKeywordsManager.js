...
// Note: KEYWORD_TYPES.label values are translated at render sites below via t().
    ...
    simplePostCallAuth(configWeb.POST_RECRUITING_KEYWORD_CREATE, JSON.stringify(payload))
      .then((res) => {
        if (!res?.error) {
          ...
          setDraft({ keyword: "", keyword_type: draft.keyword_type, weight: 1 });
          notifySuccess(t("Keyword added"));
        } else {
          notifyError(Array.isArray(res?.message) ? res.message[0] : res?.message || t("Failed to add keyword"));
        }
      })
      .catch(() => notifyError(t("Failed to add keyword")))
    ...
    simpleDeleteCallAuth(configWeb.DELETE_RECRUITING_KEYWORD(kw.id))
      .then((res) => {
        if (!res?.error) {
          ...
          notifySuccess(t("Keyword removed"));
        } else {
          notifyError(t("Failed to remove keyword"));
        }
      })
      .catch(() => notifyError(t("Failed to remove keyword")));
  };
  ...
  return (
    <div className={className} style={style}>
        ...
        <>
          <div className="d-flex align-items-center justify-content-between mb-1">
            <h6 style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 0 }}>
              <FaKey size={14} style={{ color: "#228be6" }} /> {t("AI Screening Keywords")}
            </h6>
            <span style={{ fontSize: "0.75rem", color: "#94a3b8" }}>
              {t(keywords.length === 1 ? "{{count}} keyword" : "{{count}} keywords", { count: keywords.length })}
            </span>
          </div>
          <p style={{ fontSize: "0.78rem", color: "#64748b", marginBottom: 14 }}>
            {description ||
              (live
                ? t("Add weighted keywords used to auto-score candidate CVs and applications.")
                : t("Add weighted keywords used to auto-score candidate CVs and applications. Keywords will be saved when you publish the job."))}
          </p>
        </>
      )}
      ...
      <Row className="g-2 align-items-end">
        <Col md={5}>
          <Form.Label className="fw-semibold" style={{ fontSize: "0.78rem", color: "#475569" }}>{t("Keyword")}</Form.Label>
          <Form.Control
            value={draft.keyword}
        ...
        <Col md={3}>
          <Form.Label className="fw-semibold" style={{ fontSize: "0.78rem", color: "#475569" }}>{t("Type")}</Form.Label>
          <Form.Select
          ...
          >
            {KEYWORD_TYPES.map((kt) => (
              <option key={kt.value} value={kt.value}>{t(kt.label)}</option>
            ))}
          </Form.Select>
        </Col>
        <Col md={2}>
          <Form.Label className="fw-semibold" style={{ fontSize: "0.78rem", color: "#475569" }}>{t("Weight")}</Form.Label>
          <Form.Control
            type="number"
            min="1"
            max="10"
        ...
        <Col md={2}>
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
          >
            {saving ? <Spinner size="sm" /> : <><FaPlus size={11} /> {t("Add")}</>}
          </motion.button>
        </Col>
      </Row>
