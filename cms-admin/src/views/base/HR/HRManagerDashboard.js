...
const HRManagerDashboard = () => {
  ...
  const ringData = useMemo(() => {
    ...
    return byStatus.map((r) => ({
      ...
      tooltip: `${APPLICATION_STATUS[r.status]?.label || t("Unknown")}: ${r.count}`,
    }));
  }, [stats]);
  ...
  const interviewRing = useMemo(() => {
    ...
    return byStatus.map((r) => ({
      ...
      tooltip: `${INTERVIEW_STATUS[r.status]?.label || t("Unknown")}: ${r.count}`,
    }));
  }, [interviewStats]);
  ...
  const statCards = [
    { label: t("Total Applications"), value: stats.total_applications || 0, icon: <FaUsers />, variant: "blue", change: stats.recent_applications_7days || 0, changeSuffix: t("this week") },
    { label: t("Active Jobs"), value: stats.active_jobs || 0, icon: <FaBriefcase />, variant: "green", change: stats.total_jobs || 0, changeSuffix: t("total") },
    { label: t("Hired"), value: stats.total_hired || 0, icon: <FaUserCheck />, variant: "teal", change: null, changeSuffix: t("all time") },
    { label: t("Rejected"), value: stats.total_rejected || 0, icon: <FaTimesCircle />, variant: "rose" },
  ];
    ...
    if (active && payload && payload.length) {
      return (
        <div style={{ background: "#1e293b", borderRadius: 10, padding: "10px 14px", border: "none", boxShadow: "0 8px 24px rgba(0,0,0,0.2)" }}>
          ...
          <div style={{ color: "#fff", fontSize: "0.9rem", fontWeight: 700 }}>{t("{{count}} applications", { count: payload[0].value })}</div>
        </div>
      );
    }
    return null;
        ...
        <Row className="align-items-center">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4 style={{ marginBottom: 2 }}>{t("Overview")}</h4>
                <p>{t("HR Recruiting Analytics & Insights")}</p>
              </div>
            </div>
          </Col>
          <Col xs="auto">
            <div style={{ display: "flex", alignItems: "center", gap: 12, background: "rgba(255,255,255,0.08)", borderRadius: 12, padding: "8px 16px", backdropFilter: "blur(4px)" }}>
              <div style={{ textAlign: "right" }}>
                <div style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.7rem", fontWeight: 500 }}>{t("Last 30 days")}</div>
                <div style={{ color: "#fff", fontSize: "1.15rem", fontWeight: 800 }}>{stats.recent_applications_30days || 0}</div>
              </div>
              <div style={{ width: 1, height: 32, background: "rgba(255,255,255,0.15)" }} />
              <div style={{ textAlign: "right" }}>
                <div style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.7rem", fontWeight: 500 }}>{t("Active Jobs")}</div>
          ...
          <motion.div className="hr-dash-card h-100" variants={fadeUp} custom={6} initial="hidden" animate="visible">
            <div className="d-flex justify-content-between align-items-center mb-2">
              <div className="card-title mb-0">
                <FaChartBar size={16} />{t("Application Trends")}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ fontSize: "0.72rem", color: "#94a3b8" }}>{t("Last 30 days")}</span>
                <span style={{ background: "#e7f5ff", color: "#228be6", padding: "4px 12px", borderRadius: 20, fontSize: "0.78rem", fontWeight: 700 }}>
                  {t("{{count}} total", { count: trendChartData.reduce((s, d) => s + d.count, 0) })}
                </span>
              </div>
            </div>
        ...
        <Col lg={4}>
          <motion.div className="hr-dash-card h-100" variants={fadeUp} custom={7} initial="hidden" animate="visible">
            <div className="card-title">
              <FaChartPie size={16} />{t("Application Status")}
            </div>
              ...
              <RingProgress
                ...
                label={
                  <div style={{ textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: "0.72rem", color: "#94a3b8", marginTop: 4 }}>{t("Total")}</div>
                  </div>
                }
              />
            </div>
        ...
        <Row className="mx-0 mt-3">
          <Col>
            <motion.div className="hr-dash-card" variants={fadeUp} custom={8} initial="hidden" animate="visible">
              <div className="card-title">
                <FaBriefcase size={16} />{t("Hiring Pipeline")}
              </div>
                ...
                <table className="hr-table">
                  <thead>
                    <tr>
                      ...
                      <th style={{ minWidth: 220 }}>{t("Pipeline")}</th>
                      <th style={{ textAlign: "center" }}>{t("Pending")}</th>
                      <th style={{ textAlign: "center" }}>{t("Reviewing")}</th>
                      <th style={{ textAlign: "center" }}>{t("Shortlisted")}</th>
                      <th style={{ textAlign: "center" }}>{t("Rejected")}</th>
                      <th style={{ textAlign: "center" }}>{t("Hired")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {pipeline.map((job) => {
      ...
      <Row className="mx-0 mt-3 g-3">
        ...
        <Col lg={12}>
          <motion.div className="hr-dash-card h-100" variants={fadeUp} custom={10} initial="hidden" animate="visible">
            <div className="card-title">
              <FaTrophy size={16} style={{ color: "#f59e0b" }} />{t("Top Rated Applicants")}
            </div>
            {topRated.length === 0 ? (
              <div className="hr-empty-state" style={{ minHeight: 140 }}>
                <FaTrophy size={32} style={{ color: "#e2e8f0" }} />
                <p>{t("No rated applicants yet")}</p>
              </div>
            ) : (
        ...
        <Col lg={12}>
          <motion.div className="hr-dash-card" variants={fadeUp} custom={11} initial="hidden" animate="visible">
            <div className="d-flex justify-content-between align-items-center mb-3">
              <div className="card-title mb-0">
                <FaUsers size={16} />{t("Recent Applications")}
              </div>
              <motion.button
                style={{
                ...
                whileTap={{ scale: 0.97 }}
              >
                {t("View All")} <FaArrowRight size={11} />
              </motion.button>
            </div>
            {recentApps.length === 0 ? (
              <div className="hr-empty-state" style={{ minHeight: 100 }}><FaUsers size={28} /><p>{t("No recent applications")}</p></div>
            ) : (
