...
const HRStatusHistory = () => {
  ...
  const loadItems = () => {
    ...
    simpleGetCallAuth(`${configWeb.GET_RECRUITING_STATUS_HISTORY}?${params.toString()}`)
      ...
      .catch(() => {
        notifyError(t("Failed to load status history"));
        setItems([]);
      })
  ...
  const onCreate = () => {
    if (!form.application_id) {
      notifyError(t("Application is required"));
      return;
    }
    setSaving(true);
    const payload = {
    ...
    simplePostCallAuth(configWeb.POST_RECRUITING_STATUS_HISTORY, JSON.stringify(payload))
      .then((res) => {
        if (!res?.error) {
          notifySuccess(t("Status history entry created"));
          formHandlers.close();
          loadItems();
        } else {
          notifyError(res?.message || t("Failed to create status history entry"));
        }
      })
      .catch(() => notifyError(t("Failed to create status history entry")))
    ...
    simpleGetCallAuth(configWeb.GET_RECRUITING_STATUS_HISTORY_DETAILS(id))
      .then((res) => {
        if (!res?.error || res?.id) {
          ...
          notifyError(res?.message || t("Failed to load status history details"));
        }
      })
      .catch(() => notifyError(t("Failed to load status history details")));
  };
  ...
  return (
    <div className="hr-module">
        ...
        <Row className="align-items-center g-3">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4>{t("Status History")}</h4>
                <p>{t("Track application status transitions and notes")}</p>
              </div>
            </div>
          </Col>
          <Col xs="auto" className="d-flex align-items-center gap-2 flex-wrap">
            <HRExportButtons
              ...
              title={t("HR Application Status History")}
              sheetName={t("History")}
              columns={[
                { header: t("ID"), accessor: (r) => r.id },
                { header: t("Application ID"), accessor: (r) => r.application_id },
                { header: t("Applicant"), accessor: (r) => getApplicantName(r.application || {}) },
                { header: t("From"), accessor: (r) => APPLICATION_STATUS[r.from_status]?.label || "" },
                { header: t("To"), accessor: (r) => APPLICATION_STATUS[r.to_status]?.label || "" },
                { header: t("Notes"), accessor: (r) => r.notes || "" },
                { header: t("Changed At"), accessor: (r) => formatDateTime(r.created_at) },
              ]}
            />
            <button className="hr-pill-button primary" onClick={openCreate}>
              <FaPlus size={12} /> {t("Add History Entry")}
            </button>
          </Col>
        </Row>
      </div>
        ...
        <Row className="align-items-end g-3">
          <Col lg={4} md={6}>
            <label className="form-label fw-semibold" style={{ fontSize: "0.78rem", color: "#64748b" }}>
              <FaFilter size={10} className="me-1" /> {t("Application")}
            </label>
          ...
          <Col lg={4} md={8}>
            <div style={{ position: "relative" }}>
              ...
              <Form.Control
                type="text"
                placeholder={t("Search by notes, application ID...")}
                value={searchTerm}
          ...
          <table className="hr-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7}><div className="hr-empty-state"><FaHistory size={34} /><p>{t("No status history found")}</p></div></td></tr>
              ) : (
                paginated.map((row) => {
                  ...
                  return (
                    <tr key={row.id}>
                      ...
                      <td style={{ textAlign: "center" }}>
                        <button className="hr-action-btn view" onClick={() => onOpenDetails(row.id)} title={t("View")}><FaEye /></button>
                      </td>
                    </tr>
                  );
                })
      ...
      <Modal opened={openForm} onClose={formHandlers.close} title={t("Create Status History Entry")} centered radius="lg" size="lg">
        <Row className="g-3">
          <Col md={12}>
      ...
      <Modal opened={openDetails} onClose={detailHandlers.close} title={t("Status History Details")} centered radius="lg" size="md">
        {detailData ? (
          <div>
            <p><strong>{t("ID:")}</strong> #{detailData.id}</p>
            <p><strong>{t("Application:")}</strong> #{detailData.application_id || "-"}</p>
            <p><strong>{t("From:")}</strong> {APPLICATION_STATUS[detailData.from_status]?.label || detailData.from_status}</p>
            <p><strong>{t("To:")}</strong> {APPLICATION_STATUS[detailData.to_status]?.label || detailData.to_status}</p>
            <p><strong>{t("Notes:")}</strong> {detailData.notes || "-"}</p>
            <p><strong>{t("Created:")}</strong> {formatDateTime(detailData.created_at)}</p>
          </div>
        ) : (
