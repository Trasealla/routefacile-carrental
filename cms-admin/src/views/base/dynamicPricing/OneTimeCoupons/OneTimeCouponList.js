...
const OneTimeCouponList = () => {
  ...
  const fetchList = () => {
    ...
    simpleGetCallAuth(url)
      ...
      .catch(() => {
        notifyError(t("Could not load coupons. Please try again."));
        setCoupons([]);
      })
  ...
  const onDelete = () => {    if (!delId) return;
    ...
    simpleDeleteCallAuth(configWeb.DELETE_ONE_TIME_COUPON(delId))
      ...
      .catch(() => notifyError(t("Delete failed. Please try again.")))
    ...
    <div className="otc-page">
        ...
        <div className="otc-hero-inner">
          <div>
            <span className="otc-hero-pill">
              <TfiTicket /> {t("Single-use campaign")}
            </span>
            <h1>
              {t("One-Time")} <span className="otc-hero-accent">{t("Discount Coupons")}</span>
            </h1>
            <p>
              {t("Coupons that become invalid after their usage limit is reached. Perfect for VIP campaigns, partner giveaways and one-shot codes.")}
            </p>
          </div>
          <div className="otc-hero-actions">
            <Link to="/admin/one-time-coupons/new" className="otc-btn otc-btn-primary">
              <TfiPlus /> {t("Add coupon")}
            </Link>
      ...
      <div className="otc-stats">
        <div className="otc-stat">
          ...
          <div>
            <div className="otc-stat-label">{t("Total")}</div>
        ...
        <div className="otc-stat">
          ...
          <div>
            <div className="otc-stat-label">{t("Active")}</div>
        ...
        <div className="otc-stat">
          ...
          <div>
            <div className="otc-stat-label">{t("Used up")}</div>
        ...
        <div className="otc-stat">
          ...
          <div>
            <div className="otc-stat-label">{t("Expired")}</div>
        ...
        <div className="otc-card-header">
          <div>
            <p className="otc-card-title">
              {t("{{type}} one-time coupons", { type: type === "daily" ? t("Daily") : t("Monthly") })}
            </p>
            <p className="otc-card-sub">
              {totalRecords === 0
                ? t("0 entries")
                : t("Page {{page}} · showing {{showing}} of {{filtered}} · {{total}} total", {
                    page: currentPage,
          ...
          <button
            ...
            title={
              filtered.length === 0
                ? t("No rows to export")
                : t("Download {{count}} row(s) as CSV", { count: filtered.length })
            }
          >
            <TfiDownload /> {t("Export CSV")}
          </button>
        </div>

        {loading ? (
          <div className="otc-loader">
            <Spinner animation="border" size="sm" /> {t("Loading coupons…")}
          </div>
        ) : coupons.length === 0 ? (
          <div className="otc-empty">
            <div className="otc-empty-ico"><TfiTicket /></div>
            <h3>{t("No one-time coupons yet")}</h3>
            <p>{t("Create one or generate a batch to launch your campaign.")}</p>
          ...
          <div className="otc-empty">
            ...
            <h3>{t("No matches")}</h3>
            <p>{t("Adjust your filters or search term.")}</p>
          </div>
        ) : (
