...
const OneTimeCouponBulk = () => {
  ...
  const validate = () => {
    ...
    if (tab === "generate") {
      if (!count || count < 1 || count > 5000) e.count = t("Count must be 1..5000");
      if (codeLength < 4 || codeLength > 32) e.code_length = t("Code length must be 4..32");
      if (prefix && !/^[A-Za-z0-9_-]{0,32}$/.test(prefix))
        e.prefix = t("Prefix may contain A-Z, a-z, 0-9, _ and -");
      if (prefix && prefix.length >= codeLength)
        e.prefix = t("Prefix must be shorter than code length");
    } else {
      if (parsedCodes.length === 0) e.codes = t("Paste at least one code");
      if (parsedCodes.length > 5000) e.codes = t("Maximum 5000 codes per batch");
    }
  ...
  const onSubmit = (event) => {
    ...
    simplePostCallAuth(configWeb.POST_ONE_TIME_COUPON_BULK, JSON.stringify(payload))
      .then((res) => {
        ...
        notifySuccess(t("Created {{count}} coupon(s)", { count: created.length }));
      })
      .catch(() => notifyError(t("Bulk create failed. Please try again.")))
  ...
  const resetForm = () => {
    if (
      ...
      !csvDownloaded &&
      !window.confirm(
        t("You have not downloaded the CSV of created codes. Continue anyway?")
      )
    )
      return;
  ...
  const goBack = (e) => {
    if (
      ...
      !csvDownloaded &&
      !window.confirm(
        t("You have not downloaded the CSV of created codes. Leave anyway?")
      )
    ) {
      e.preventDefault();
      return;
        ...
        <div className="otc-hero-inner">
          <div>
            <span className="otc-hero-pill">
              <TfiLayersAlt /> {t("Bulk create")}
            </span>
            <h1>
              {t("Generate")} <span className="otc-hero-accent">{t("campaigns at scale")}</span>
            </h1>
            <p>
              {t("Create up to 5000 single-use coupons in one shot. Generate codes automatically or paste your own list, then download the CSV.")}
            </p>
          </div>
            ...
            <div className="otc-card-header" style={{ background: "linear-gradient(180deg,#ecfdf5,#fff)" }}>
              <div>
                <p className="otc-card-title" style={{ color: "#065f46" }}>
                  <TfiCheck /> {t("Batch processed")}
                </p>
                <p className="otc-card-sub">
                  {t("{{created}} created · {{skipped}} skipped", { created: result.created.length, skipped: result.skipped.length })}
                </p>
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                >
                  <TfiDownload /> {t("Download created (CSV)")}
                </button>
                <button
                  type="button"
                ...
                >
                  <TfiDownload /> {t("Download skipped")}
                </button>
              </div>
            </div>
            <div className="otc-card-body">
              <div className="otc-result-grid">
                <div className="otc-result-card is-success">
                  <div className="otc-result-head">
                    <span><TfiCheck /> {t("Created ({{count}})", { count: result.created.length })}</span>
                  </div>
                  <div className="otc-result-body">
                    {result.created.length === 0 ? (
                      <div className="otc-empty" style={{ padding: 20 }}>
                        <p>{t("No new coupons were created in this batch.")}</p>
                      </div>
                    ) : (
                      <table>
                        <tbody>
                ...
                <div className="otc-result-card is-warn">
                  <div className="otc-result-head">
                    <span><TfiAlert /> {t("Skipped ({{count}})", { count: result.skipped.length })}</span>
                  </div>
                  <div className="otc-result-body">
                    {result.skipped.length === 0 ? (
                      <div className="otc-empty" style={{ padding: 20 }}>
                        <p>{t("Nothing was skipped — clean run.")}</p>
                      </div>
                    ) : (
                      <table>
                        <tbody>
              ...
              <div className="otc-footer-actions" style={{ marginTop: 18 }}>
                <button type="button" className="otc-btn otc-btn-outline" onClick={resetForm}>
                  <TfiPlus /> {t("Create another batch")}
                </button>
                <Link
        ...
        <form id="otc-bulk-form" onSubmit={onSubmit} noValidate>
          ...
          <div className="otc-card otc-section">
            <div className="otc-card-header">
              <div>
                <p className="otc-card-title">{t("Codes")}</p>
                <p className="otc-card-sub">
                  {t("Auto-generate random codes or paste your own list.")}
                </p>
              </div>
            </div>
            <div className="otc-card-body">
              ...
              {tab === "generate" ? (
                <div className="otc-form-grid">
                  <div className="otc-field span-3">
                    <label className="otc-label">
                      {t("Count")}<span className="req">*</span>
                    </label>
                    <input
                      className={`otc-input ${
                    ...
                    />
                    <div className="otc-help">{t("1 to 5000 codes per batch.")}</div>
                    {validated && errors.count && (
                      <div className="otc-error">{errors.count}</div>
                    )}
                  </div>
                  <div className="otc-field span-3">
                    <label className="otc-label">{t("Prefix")}</label>
                    <input
                      className={`otc-input ${
                      ...
                      type="text"
                      maxLength={32}
                      placeholder={t("e.g. MAY26-")}
                      value={prefix}
                      onChange={(e) => setPrefix(e.target.value)}
                    />
                    <div className="otc-help">{t("A–Z, a–z, 0–9, _ and -.")}</div>
                    {validated && errors.prefix && (
                      <div className="otc-error">{errors.prefix}</div>
                    )}
                  </div>
                  <div className="otc-field span-3">
                    <label className="otc-label">{t("Code length")}</label>
                    <input
                      className={`otc-input ${
                    ...
                    />
                    <div className="otc-help">{t("Total length 4 to 32.")}</div>
                    {validated && errors.code_length && (
                      <div className="otc-error">{errors.code_length}</div>
                    )}
                  </div>
                  <div className="otc-field span-3">
                    <label className="otc-label">{t("Suffix charset")}</label>
                    <select
                      className="otc-input"
                      value={charset}
                      onChange={(e) => setCharset(e.target.value)}
                    >
                      <option value="alphanumeric">{t("Alphanumeric (A-Z, 0-9)")}</option>
                      <option value="alpha">{t("Letters only (A-Z)")}</option>
                      <option value="numeric">{t("Numbers only (0-9)")}</option>
                    </select>
                    <div className="otc-help">
                      {t("Charset used for the auto-generated portion after the prefix.")}
                    </div>
                  </div>
                  <div className="otc-field span-3">
                    <label className="otc-label">&nbsp;</label>
                    <div className="otc-preview">
                      <span className="lbl"><TfiInfoAlt /> {t("Live preview")}</span>
                      {examples.map((ex, i) => (
                ...
                <div className="otc-form-grid">
                  <div className="otc-field span-12">
                    <label className="otc-label">
                      {t("Paste codes")}<span className="req">*</span>
                    </label>
                    <textarea
                    ...
                    />
                    <div className="otc-help">
                      {t("One per line, or comma/semicolon separated. Whitespace is trimmed and duplicates are de-duplicated.")}
                    </div>
                    <div
                      className="otc-help"
                      style={{
                    ...
                    >
                      <strong>{parsedCodes.length}</strong>{" "}
                      {parsedCodes.length === 1 ? t("unique code detected") : t("unique codes detected")}
                      {parsedCodes.length > 5000 ? t(" — exceeds maximum (5000)") : ""}.
                    </div>
          ...
          <div className="otc-card otc-section">
            <div className="otc-card-header">
              <div>
                <p className="otc-card-title">{t("Settings (applied to every code)")}</p>
                <p className="otc-card-sub">
                  {t("These values are written to each coupon in the batch.")}
                </p>
              </div>
            </div>
            <div className="otc-card-body">
              <div className="otc-form-grid">
                <div className="otc-field span-3">
                  <label className="otc-label">{t("Type")}</label>
                  <select
                ...
                <div className="otc-field span-3">
                  <label className="otc-label">{t("Discount type")}</label>
                  <select
                    className="otc-select"
                ...
                <div className="otc-field span-3">
                  <label className="otc-label">
                    {t("Usage limit per code")}<span className="req">*</span>
                  </label>
                  <input
                    className={`otc-input ${
                  ...
                  />
                  <div className="otc-help">{t("Default 1 (single-use).")}</div>
          ...
          <div className="otc-card otc-section">
            ...
            <div className="otc-card-body">
              <div className="otc-form-grid">
                <div className="otc-field span-6">
                  <label className="otc-label">{t("Cities")}</label>
                  <Select
                    isMulti
                    options={cityOpts}
                ...
                <div className="otc-field span-6">
                  <label className="otc-label">{t("Locations")}</label>
                  <Select
                    isMulti
                ...
                <div className="otc-field span-6">
                  <label className="otc-label">{t("Car groups")}</label>
                  <Select
                    isMulti
                    options={groupOpts}
                ...
                <div className="otc-field span-6">
                  <label className="otc-label">{t("Cars")}</label>
                  <Select
                    isMulti
                    options={carOpts}
                ...
                <div className="otc-field span-12">
                  <label className="otc-label">{t("Note (optional)")}</label>
                  <textarea
                    className="otc-textarea"
                    placeholder={t("Campaign reference, internal note…")}
                    value={formData.note}
