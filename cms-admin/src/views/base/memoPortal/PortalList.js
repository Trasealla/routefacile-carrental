    ...
    <PortalLayout embedded={embedded} email={email}>
      ...
      >
        ...
        <div className="mp-hero__content">
          <div>
            <span className="mp-hero__eyebrow">{t("Memo Portal")}</span>
            <h1 className="mp-hero__title">
              {t("Welcome")}{email ? `, ${email.split("@")[0].split(".")[0]}` : ""}.
              <br />
              <span>{t("Stay in the loop.")}</span>
            </h1>
            <p className="mp-hero__sub">
              {t("Browse the latest published memos, circulars and policies shared with you.")}
            </p>
          </div>
          <div className="mp-hero__stats">
            <div className="mp-stat">
              ...
              <div>
                ...
                <div className="mp-stat__lbl">{t("Available")}</div>
              </div>
            </div>
            <div className="mp-stat">
              ...
              <div>
                ...
                <div className="mp-stat__lbl">{t("New (7d)")}</div>
              </div>
            </div>
            <div className="mp-stat">
              ...
              <div>
                ...
                <div className="mp-stat__lbl">{t("Categories")}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {(q || activeCategoryName) && !loading && (
        <div className="mp-activeFilters">
          <span className="mp-activeFilters__lbl">{t("Showing:")}</span>
          {activeCategoryName && (
            <span className="mp-activeFilters__chip">
              <FaFolderOpen size={10} /> {activeCategoryName}
              <button onClick={() => setCategoryId("")} aria-label={t("Remove category filter")}>×</button>
            </span>
          )}
          {q && (
            <span className="mp-activeFilters__chip">
              <FaSearch size={10} /> "{q}"
              <button onClick={() => setQ("")} aria-label={t("Remove search filter")}>×</button>
            </span>
          )}
        </div>
      )}
      ...
      {loading ? (
        <div className="mp-stateBox">
          <Loader color="indigo" />
          <p className="mp-stateBox__sub">{t("Loading memos…")}</p>
        </div>
      ) : error ? (
        <motion.div
        ...
        >
          ...
          <h3>{t("Could not load memos")}</h3>
          <p className="mp-stateBox__sub">{error.message || t("Please try again.")}</p>
          <button className="mp-btn mp-btn--primary" onClick={load}>{t("Retry")}</button>
        </motion.div>
      ) : items.length === 0 ? (
        <motion.div
          initial={{ opacity: 0, y: 8 }}
        ...
        >
          ...
          <h3>{q || categoryId ? t("No memos match your filters") : t("No memos available yet")}</h3>
          <p className="mp-stateBox__sub">
            {q || categoryId
              ? t("Try clearing your filters or searching with different keywords.")
              : t("New memos will appear here as they are published to you.")}
          </p>
          {(q || categoryId) && (
            ...
            return (
              ...
              >
                <div className="mp-card__top" style={{ background: bg }}>
                  ...
                  {isNew && <span className="mp-card__newBadge">{t("NEW")}</span>}
                </div>
      ...
      {!loading && !error && items.length >= PAGE_SIZE && (
        <div className="mp-pagination">
          ...
          <span className="mp-pagination__info">{t("Page {{page}}", { page })}</span>
