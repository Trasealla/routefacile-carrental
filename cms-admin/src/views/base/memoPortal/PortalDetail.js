...
const PortalDetail = ({ embedded = false }) => {
    ...
    portalApi
      ...
      .catch((e) => {
        if (alive) {
          setPreviewError(
            e?.status === 404
              ? t("This memo is no longer available or you no longer have access.")
              : e?.message || t("Failed to load file. Please try again.")
          );
        }
      })
  ...
  return (
    <PortalLayout embedded={embedded} email={email}>
      ...
      >
        <FaArrowLeft size={11} /> {t("All memos")}
      </button>
      ...
      {loading ? (
        ...
        >
          ...
          <h5 className="mt-2">{t("Memo unavailable")}</h5>
          <p className="text-muted small mb-3">
            {error?.message || t("This memo is no longer available.")}
          </p>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="mp-detail-hero">
            ...
            <div className="mp-meta">
              ...
              <span>
                <FaEye size={10} />{" "}
                {t("{{count}} views", { count: (liveViewCount ?? doc.view_count ?? doc.viewCount ?? doc.views ?? 0) })}
              </span>
              {tags.length > 0 && (
          ...
          <div className="mp-preview">
            {previewLoading ? (
              <div className="mp-preview__empty">
                <Loader color="indigo" />
                <p className="text-muted small mt-2 mb-0">{t("Loading file…")}</p>
              </div>
            ) : previewError ? (
              <div className="mp-preview__empty">
                <FaExclamationTriangle size={28} style={{ color: "#ef4444" }} />
                <h5 className="mt-2">{t("Could not load file")}</h5>
                <p className="text-muted small mb-0">{previewError}</p>
              </div>
            ) : !blobUrl ? (
              <div className="mp-preview__empty">
                <FaFileAlt size={28} style={{ color: "#94a3b8" }} />
                <p className="text-muted small mb-0">{t("No file attached.")}</p>
              </div>
            ) : isImage ? (
              <img
                src={blobUrl}
              ...
              <div className="mp-preview__empty">
                ...
                <h5 className="mt-2">{t("Preview not supported")}</h5>
                <p className="text-muted small">
                  {t("This file type cannot be previewed in the browser. Download it to view.")}
                </p>
                <button className="mp-btn mp-btn--primary" onClick={handleDownload}>
                  <FaDownload size={11} /> {t("Download file")}
                </button>
              </div>
            ) : (
              <iframe
