...
const NewDesignVote = () => {
    ...
    simpleGetCall(url)
      .then((res) => {
        if (!res?.error) {
          ...
          setVoteData([]);
          setTotalRecords(0);
          notifyError(res?.message || t("Failed to load vote data"));
        }
      })
      .catch((error) => {
  ...
  const getChoiceText = (choice) => {
    if (choice === "first") return t("New UI");
    if (choice === "second") return t("Old UI");
    return choice || "-";
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    if (!jsPDF || !autoTable) {
      try {
        ...
        notifyError(t("PDF generation is not available. Please contact support."));
        return;
      }
    }
    ...
    if (allVoteDataForPDF.length === 0) {
      ...
      try {
        ...
        if (response?.data && Array.isArray(response.data)) {
          ...
          notifyError(response?.message || t("Failed to fetch all vote data for PDF export"));
          setLoading(false);
          return;
        }
      } catch (error) {
        console.error("Error fetching all vote data:", error);
        notifyError(t("Failed to fetch all vote data. Please try again later."));
        setLoading(false);
        return;
      } finally {
        setLoading(false);
    ...
    doc.save(fileName);
    notifySuccess(t("PDF exported successfully!"));
  };
  ...
  const handleDownloadExcel = async () => {
    ...
    if (allVoteDataForExcel.length === 0) {
      ...
      try {
        ...
        if (response?.data && Array.isArray(response.data)) {
          ...
          notifyError(response?.message || t("Failed to fetch all vote data for Excel export"));
          setLoading(false);
          return;
        }
      } catch (error) {
    ...
    if (allVoteDataForExcel.length === 0) {
      notifyError(t("No data available to export"));
      return;
    }
    ...
    notifySuccess(t("Excel file exported successfully!"));
  };
      ...
      <Row className="mb-4">
        <Col>
          <div className="d-flex justify-content-between align-items-center">
            <div>
              <h3>{t("New Design Vote")}</h3>
              <p className="text-muted mb-0">{t("View user votes for UI preference")}</p>
            </div>
      ...
      <Row className="mb-4">
        <Col md={4} sm={12} className="mb-3">
          <Card className="h-100 shadow-sm" style={{ borderLeft: '4px solid #53b7e8', cursor: 'pointer' }}>
            <Card.Body>
              <Card.Title className="text-muted small mb-2">{t("Total Votes")}</Card.Title>
              <Card.Text 
                className="h3 mb-0" 
                style={{ 
                  color: '#53b7e8',   
        ...
        <Col md={4} sm={12} className="mb-3">
          <Card className="h-100 shadow-sm" style={{ borderLeft: '4px solid #28a745', cursor: 'pointer' }}>
            <Card.Body>
              <Card.Title className="text-muted small mb-2">{t("New UI")}</Card.Title>
              <Card.Text 
                className="h3 mb-0" 
                style={{ 
                  color: '#28a745', 
        ...
        <Col md={4} sm={12} className="mb-3">
          <Card className="h-100 shadow-sm" style={{ borderLeft: '4px solid #17a2b8', cursor: 'pointer' }}>
            <Card.Body>
              <Card.Title className="text-muted small mb-2">{t("Old UI")}</Card.Title>
              <Card.Text 
                className="h3 mb-0" 
                style={{ 
                  color: '#17a2b8', 
