...
const DownloadReport = () => {
  ...
  const handleFilter = () => {
    if (!fromDate || !toDate) {
      notifyError(t("Please select both From and To dates."));
      return;
    }
    if (new Date(fromDate) > new Date(toDate)) {
      notifyError(t("From date cannot be after To date."));
      return;
    }
  ...
  const handleExport = () => {
    ...
    try {
      ...
      if (filtered.length === 0) {
        notifyError(t("No bookings found for the selected date range."));
      ...
      const rows = filtered.map((b, i) => ({
        "Sr. No": i + 1,
        "Booking No": b.arc_number || b.booking_number || "-",
        ...
        "Payment Reference": b.payfort_id || "-",
      ...
      notifyError(t("Export failed. Please try again."));
    } finally {
      setExportLoading(false);
    }
  };
      ...
      <div className="report-page">
        <div className="d-flex justify-content-center align-items-center" style={{ minHeight: "80vh" }}>
          <div className="text-center">
            ...
            <p className="mt-3 text-muted">{t("Generating report...")}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="report-page">
        ...
        <Row className="align-items-center">
          <Col>
            <div className="d-flex align-items-center">
              ...
              <div>
                <h4 className="report-title mb-0">{t("Booking & Financial Report")}</h4>
          ...
          <Col xs="auto" className="d-flex gap-2 no-print">
            <Button
              ...
              onClick={handlePrint}
            >
              <FaPrint className="me-1" /> {t("Print")}
            </Button>
          </Col>
        </Row>
        {/* Export Controls */}
        <div className="report-header-export mt-3 no-print">
          <div className="d-flex align-items-end gap-3 flex-wrap">
            <div style={{ minWidth: 170 }}>
              <label className="form-label small mb-1" style={{ color: "rgba(255,255,255,0.75)" }}>{t("From Date")}</label>
              <input
                type="date"
            ...
            <div style={{ minWidth: 170 }}>
              <label className="form-label small mb-1" style={{ color: "rgba(255,255,255,0.75)" }}>{t("To Date")}</label>
              <input
                type="date"
                className="form-control form-control-sm report-date-input"
                value={toDate}
      ...
      <Row className="mb-3">
        {[
          { label: t("Total Bookings"), value: stats.totalBookings, icon: <FaCalendarCheck size={20} />, color: "#1E3A8A" },
          { label: t("Total Revenue"), value: `MAD ${Number(stats.totalRevenue).toLocaleString()}`, icon: <FaMoneyBillWave size={20} />, color: "#059669" },
          { label: t("Pay Now"), value: stats.payNow, icon: <FaCreditCard size={20} />, color: "#2563EB" },
          { label: t("Pay Later"), value: stats.payLater, icon: <FaHandHoldingUsd size={20} />, color: "#D97706" },
          { label: t("Completed"), value: stats.completed, icon: <FaCheckCircle size={20} />, color: "#10B981" },
          { label: t("Cancelled"), value: stats.cancelled, icon: <FaUndoAlt size={20} />, color: "#EF4444" },
          { label: t("Incomplete"), value: stats.incompleteBookings, icon: <FaExclamationTriangle size={20} />, color: "#F59E0B" },
          { label: t("Failed"), value: stats.failed, icon: <FaTimesCircle size={20} />, color: "#DC2626" },
        ].map((item, i) => (
          ...
          <Card className="report-chart-card">
            <CardBody>
              <div className="d-flex align-items-center mb-3">
                ...
                <h5 className="report-chart-title mb-0">{t("Daily Booking Trend (Last 14 Days)")}</h5>
              </div>
      ...
      <Row className="mb-3">
        <Col lg={7} className="mb-3">
          <Card className="report-chart-card h-100">
            <CardBody>
              <h5 className="report-chart-title mb-3">{t("Monthly Bookings (Last 6 Months)")}</h5>
          ...
          <Card className="report-chart-card h-100">
            <CardBody>
              <div className="d-flex align-items-center mb-3">
                ...
                <h5 className="report-chart-title mb-0">{t("Payment Breakdown")}</h5>
              </div>
              <div style={{ height: "280px" }}>
      ...
      <Row className="mb-3">
        <Col lg={5} className="mb-3">
          <Card className="report-chart-card h-100">
            <CardBody>
              <h5 className="report-chart-title mb-3">{t("Booking Status Distribution")}</h5>
              <div style={{ height: "280px" }}>
        ...
        <Col lg={7} className="mb-3">
          <Card className="report-chart-card h-100">
            <CardBody>
              <h5 className="report-chart-title mb-3">{t("Summary Table")}</h5>
              <Table bordered hover responsive className="report-summary-table mb-0">
                ...
                <tbody>
                  ...
                  <tr>
                    <td>{t("Pay Now Bookings")}</td>
                    <td className="text-end fw-semibold">{stats.payNow}</td>
                  </tr>
                  <tr>
                    <td>{t("Pay Later Bookings")}</td>
          ...
          <Card className="report-chart-card">
            <CardBody>
              <div className="d-flex justify-content-between align-items-center mb-3">
                <h5 className="report-chart-title mb-0">{t("All Bookings ({{count}})", { count: filteredBookings.length })}</h5>
                <small className="text-muted">
                  {fromDate && toDate ? t("{{from}} to {{to}}", { from: fromDate, to: toDate }) : t("All Time")} — {t("export to Excel for the complete dataset")}
                </small>
              </div>
                ...
                <Table hover className="report-bookings-table mb-0" size="sm">
                  <thead>
                    <tr>
                      ...
                      <th>{t("Car")}</th>
                      <th>{t("Pickup Type")}</th>
                      <th>{t("Pickup Location")}</th>
                      <th>{t("Pickup City")}</th>
                      <th>{t("Pickup Date")}</th>
                      <th>{t("Pickup Address")}</th>
                      <th>{t("Dropoff Type")}</th>
                      <th>{t("Dropoff Location")}</th>
                      <th>{t("Dropoff City")}</th>
                      <th>{t("Dropoff Date")}</th>
                      <th>{t("Dropoff Address")}</th>
                      <th>{t("Payment Reference")}</th>
                      <th className="text-end">{t("Car Rate")}</th>
                      <th className="text-end">{t("Inter City")}</th>
                      <th className="text-end">{t("Parking")}</th>
                      <th className="text-end">{t("VMD")}</th>
                      <th className="text-end">{t("Delivery")}</th>
                      <th className="text-end">{t("Collection")}</th>
                      <th>{t("Coupon")}</th>
                      <th className="text-end">{t("VAT")}</th>
                    ...
                    {filteredBookings.length === 0 ? (
                      <tr>
                        <td colSpan={35} className="text-center text-muted py-4">
                          {t("No bookings found for the selected date range")}
                        </td>
                      </tr>
                    ) : (
