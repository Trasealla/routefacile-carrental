...
const Bookings = () => {
  ...
  const getBookings = () => {
    ...
    if (paramsData.broker_id)
      params.append("broker_id", paramsData.broker_id);
  ...
  useEffect(() => {
    simpleGetCallAuth(`${configWeb.GET_BROKER_LIST}?page=1&page_size=1000&status=1`)
      .then((res) => {
        if (!res?.error) setBrokersArray(res?.data || []);
      })
      .catch(() => setBrokersArray([]));
  }, []);
  ...
  const calculatePaginationMessage = () => {
    if (!totalRecords) {
      return `${t("common.showing")} 0 ${t("common.to")} 0 ${t("common.of")} 0 ${t("common.entries")}`;
    }
  ...
  const translateStatus = (status) =>
    statusTranslationKeys[status] ? t(`booking.${statusTranslationKeys[status]}`) : status;
  ...
  const handleDownloadReport = async () => {
    if (downloadReportDisabled) {
      notifyError(t("booking.noDataAvailableToDownload"));
      return;
    }
    ...
    if (!jsPDF || !autoTable) {
      try {
        ...
        notifyError(t("booking.pdfNotAvailable"));
        return;
      }
    }
    ...
    const getAppliedFilters = () => {
      ...
      if (paramsData.broker_id) {
        const brokerName = brokersArray.find((b) => String(b.id) === String(paramsData.broker_id))?.name;
        filters.push(`Broker: ${brokerName || paramsData.broker_id}`);
      }
      return filters.join(" | ");
    };
    ...
    const tablePage1Columns = [
      { header: "Booking No", dataKey: "arc_number" },
      { header: "Booking ID", dataKey: "booking_log_number" },
      { header: "Source", dataKey: "source" },
      { header: "Broker Ref", dataKey: "broker_reference_display" },
    ...
    const tablePage2Columns = [
      ...
      { header: "Payment Reference", dataKey: "payfort_id" },
      ...
      // No tax is charged, so the VAT column is omitted from the PDF export.
    ...
    const tableData = priceListArray.map((item) => ({
      ...
      source: item.source === "broker" && item.broker_name ? `Broker: ${item.broker_name}` : (item.source || "-"),
      broker_reference_display: item.broker_reference || "-",
    ...
    notifySuccess(t("booking.pdfDownloaded"));
  };
  ...
  const handleDownloadExcel = async () => {
    ...
    if (!XLSX) {
      try {
        ...
        notifyError(t("booking.excelNotAvailable"));
        return;
      }
    }
    ...
    const excelData = priceListArray.map((item, index) => ({
      "Sr. No": index + 1,
      "Booking No": item.arc_number || item.booking_number || "-",
      "Booking ID": item.booking_log_number || "-",
      "Source": item.source === "broker" && item.broker_name ? `Broker: ${item.broker_name}` : (item.source || "-"),
      "Broker Reference": item.broker_reference || "-",
      ...
      "Payment Reference": item.payfort_id || "-",
      ...
      // VAT column dropped from the Excel export — no tax is charged.
    ...
    notifySuccess(t("booking.excelDownloaded"));
  };
  ...
  return (
    <Container fluid className="bookings-container">
      {/* Summary stat cards */}
      <div className="bk-summary-row mt-md">
        <div className="bk-stat-card">
          <div className="bk-stat-icon">
            <TbCalendarCheck size={22} />
          </div>
          <div>
            <div className="bk-stat-label">{t("booking.totalBookings")}</div>
            <div className="bk-stat-value">
              {(summaryData?.total_bookings ?? totalRecords ?? 0).toLocaleString()}
            </div>
          </div>
        </div>
        <div className="bk-stat-card">
          <div className="bk-stat-icon green">
            <TbCoin size={22} />
          </div>
          <div>
            <div className="bk-stat-label">{t("booking.totalRevenue")}</div>
            <div className="bk-stat-value">
              MAD {Number(summaryData?.total_revenue || 0).toLocaleString(undefined, { maximumFractionDigits: 0 })}
            </div>
          </div>
        </div>
        <div className="bk-stat-card">
          <div className="bk-stat-icon navy">
            <TbListDetails size={22} />
          </div>
          <div>
            <div className="bk-stat-label">{t("booking.showing")}</div>
            <div className="bk-stat-value">{priceListArray.length} {t("common.of")} {totalRecords}</div>
          </div>
        </div>
        <div className="bk-stat-card">
          <div className="bk-stat-icon amber">
            <TbFilter size={22} />
          </div>
          <div>
            <div className="bk-stat-label">{t("booking.activeFiltersLabel")}</div>
            <div className="bk-stat-value">{activeFilterCount}</div>
          </div>
        </div>
      </div>
          ...
          <Group gap="sm" wrap="nowrap">
            ...
            {activeFilterCount > 0 && (
              <Badge color="orange" variant="light" size="lg" radius="sm">
                {activeFilterCount} {t("booking.activeFilter", { count: activeFilterCount })}
              </Badge>
            )}
          </Group>
          <Group gap="sm" wrap="wrap">
            <MTextInput
              placeholder={t("booking.quickSearchPlaceholder")}
            ...
            <MButton
              ...
              rightSection={
                activeFilterCount > 0 ? (
                  <Badge color="orange" circle size="sm">
                    {activeFilterCount}
                  </Badge>
                ) : null
              }
            ...
            {activeFilterCount > 0 && (
              <Tooltip label={t("booking.clearAllFilters")} withArrow>
                <ActionIcon
                  variant="subtle"
                  color="gray"
                  size="lg"
                  onClick={handleResetFilters}
                  aria-label={t("booking.clearAllFilters")}
                >
                  <TbRefresh size={18} />
                </ActionIcon>
              </Tooltip>
        ...
        <div className="d-flex flex-column gap-3">
          <Button
            ...
            style={{
              width: '60px',
              height: '60px',
              background: 'linear-gradient(135deg, #f2421b 0%, #d63a17 100%)',
              border: 'none',
              boxShadow: '0 6px 16px rgba(242, 66, 27, 0.35)'
            }}
            title={t("booking.downloadPdfReport")}
          >
            <svg 
              width="24" 
              height="24" 
          ...
          <Button
            ...
            title={t("booking.downloadExcelReport")}
          >
            <svg 
              width="24" 
              height="24" 
      ...
      <Drawer
        ...
        size="lg"
        padding="lg"
        title={<Title order={4}>{t("booking.searchFilters")}</Title>}
      ...
      >
        ...
        <Row className="mb-4">
          <Col xs={12}>
            <h6 className="text-muted mb-3">{t("booking.bookingCreatedDateSection")}</h6>
          </Col>
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
          <Form.Group controlId="booking_start_date">
              <Form.Label>{t("common.from")}</Form.Label>
              <Form.Control
                type="date"
        ...
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group controlId="booking_end_date" className="mb-3">
              <Form.Label>{t("common.to")}</Form.Label>
              <Form.Control
                type="date"
        ...
        <Col xs={12} className="mt-3">
            <h6 className="text-muted mb-3">{t("booking.pickupDateSection")}</h6>
          </Col>
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
        ...
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group controlId="booking_number" className="mb-3">
              <Form.Label>{t("booking.bookingNumber")}</Form.Label>
              <Form.Control
                type="text"
                name="booking_number"
                id="booking_number"
        ...
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group controlId="user_email" className="mb-3">
              <Form.Label>{t("booking.userEmail")}</Form.Label>
              <Form.Control
                type="text"
                name="user_email"
                id="user_email"
        ...
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group className="mb-3">
              <Form.Label>{t("booking.paymentType")}</Form.Label>
              <Form.Select
      ...
      <Row>
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group className="mb-3">
              <Form.Label>{t("booking.bookingType")}</Form.Label>
              <Form.Select
                aria-label="Default select example"
                name="type"
        ...
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group className="mb-3">
              <Form.Label>{t("booking.bookingStatus")}</Form.Label>
              <Form.Select
                aria-label="Default select example"
                name="status"
              ...
              >
                ...
                <option key="edit" value="edit">
                  {t("booking.statusEdited")}
                </option>
                <option key="extend" value="extend">
                  {t("booking.statusExtended")}
                </option>
        ...
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group className="mb-3">
              <Form.Label>{t("booking.pickupType")}</Form.Label>
              <Form.Select
                aria-label="Default select example"
                name="pickup_type"
        ...
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group className="mb-3">
              <Form.Label>{t("booking.dropoffType")}</Form.Label>
              <Form.Select
                aria-label="Default select example"
                name="dropoff_type"
      ...
      <Row>
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group className="mb-3">
              <Form.Label>{t("booking.location")}</Form.Label>
              <Form.Select
                aria-label="Default select example"
                name="location_id"
        ...
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group className="mb-3">
              <Form.Label>{t("booking.bookingSource")}</Form.Label>
              <Form.Select
                aria-label="Default select example"
                name="booking_source"
              ...
              >
                <option value="">{t("common.select")}</option>
                <option value="web">{t("booking.website")}</option>
                <option value="mobile">{t("booking.mobile")}</option>
                <option value="broker">{t("booking.broker")}</option>
                <option value="api">{t("booking.api")}</option>
              </Form.Select>
            </Form.Group>
        </Col>
        ...
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
            <Form.Group className="mb-3">
              <Form.Label>{t("booking.broker")}</Form.Label>
              <Form.Select
                aria-label="Default select example"
                name="broker_id"
                value={paramsData.broker_id}
                onChange={handleParamChange}
              >
                <option value="">{t("common.select")}</option>
                {brokersArray.map((broker) => (
        ...
        <Col xs={12} md={6} lg={4} xl={3} className="mb-3">
          <Form.Group>
            <Form.Label>{t("booking.promoCode")}</Form.Label>
            <Form.Control
              type="text"
              name="coupon_code"
      ...
      <div className="results-card">
        ...
        <div className="table-responsive">
            ...
            <thead>
              <tr>
                <th scope="col">{t("booking.colSrNo")}</th>
                <th scope="col">{t("booking.colArcNumber")}</th>
                <th scope="col">{t("booking.colBookingId")}</th>
                <th scope="col">{t("booking.colSource")}</th>
                <th scope="col">{t("booking.colBrokerReference")}</th>
                <th scope="col">{t("booking.colBookingDate")}</th>
                <th scope="col">{t("common.status")}</th>
                <th scope="col">{t("booking.colType")}</th>
                <th scope="col">{t("booking.colPayment")}</th>
                <th scope="col">{t("booking.colDays")}</th>
                <th scope="col">{t("booking.colMonths")}</th>
                <th scope="col">{t("booking.colExtraDays")}</th>
                <th scope="col">{t("booking.colUserName")}</th>
                <th scope="col">{t("booking.colUserEmail")}</th>
                <th scope="col">{t("booking.colUserPhone")}</th>
                <th scope="col">{t("booking.colCar")}</th>
                <th scope="col">{t("booking.colPickup")}</th>
                <th scope="col">{t("booking.colPickupLocation")}</th>
                <th scope="col">{t("booking.colPickupCity")}</th>
                <th scope="col">{t("booking.colPickupDateTime")}</th>
                <th scope="col">{t("booking.colPickupAddress")}</th>
                <th scope="col">{t("booking.colDropoff")}</th>
                <th scope="col">{t("booking.colDropoffLocation")}</th>
                <th scope="col">{t("booking.colDropoffCity")}</th>
                <th scope="col">{t("booking.colDropoffDateTime")}</th>
                <th scope="col">{t("booking.colDropoffAddress")}</th>
                <th scope="col">{t("booking.colPayfortId")}</th>
                <th scope="col">{t("booking.colCarRate")}</th>
                <th scope="col">{t("booking.colInterCityCharges")}</th>
                <th scope="col">{t("booking.colParkingCharges")}</th>
                <th scope="col">{t("booking.colVmdCharges")}</th>
                <th scope="col">{t("booking.colDeliveryCharges")}</th>
                <th scope="col">{t("booking.colCollectCharges")}</th>
                <th scope="col">{t("booking.colCouponCode")}</th>
                <th scope="col">{t("booking.colBookingAmount")}</th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                priceListArray?.map((item, index) => (
                  <tr key={item.id}>
                    ...
                    <td>{item.source === "broker" && item.broker_name ? `${t("booking.broker")}: ${item.broker_name}` : item.source}</td>
                    <td>{item.broker_reference || "-"}</td>
                    <td>{item.booking_date}</td>
                    <td>
                      <div className={`${item.status}_box`}>{translateStatus(item.status)}</div>
                    </td>
                    <td>{item.type}</td>
