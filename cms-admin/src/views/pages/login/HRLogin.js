...
const HRLogin = () => {
    ...
    simplePostCall(
      ...
      .then((res) => {
        ...
        if (!ALLOWED_ROLES.includes(userType)) {
          const msg = t("This portal is restricted to HR users only.");
          setErrorMessage(msg);
          notifyError(msg);
          return;
        }
        ...
        localStorage.setItem(
          "routefacile_must_reset_password",
          res?.must_reset_password ? "1" : "0"
        );
        localStorage.setItem("routefacile_login_portal", "hr");

        notifySuccess(t("Welcome to the HR Portal"));
  ...
  const features = [
    { icon: <FaBriefcase />, title: t("Job Postings"), desc: t("Publish and manage open roles") },
    { icon: <FaUsers />, title: t("Candidate Pipeline"), desc: t("Track applications end-to-end") },
    { icon: <FaCalendarCheck />, title: t("Interviews"), desc: t("Schedule and rate candidates") },
    { icon: <FaChartLine />, title: t("Insights"), desc: t("Funnel and channel analytics") },
  ];
          ...
          <div className="hr-login-brand">
            <div className="hr-login-brand-top">
              ...
              <span className="hr-login-pill">
                <FaUserTie /> {t("HR Portal")}
              </span>
            </div>

            <div className="hr-login-brand-hero">
              <h1>
                {t("Hire")} <span>{t("smarter")}</span>.
                <br />
                {t("Move faster.")}
              </h1>
              <p>
                {t("One workspace for managers and recruiters to post roles, screen candidates, run interviews and make confident hiring calls.")}
              </p>
            </div>
            ...
            <div className="hr-login-brand-foot">
              ...
              <span>{t("HR Manager · HR Recruitment")}</span>
            </div>
          </div>
          ...
          <div className="hr-login-form-side">
            <div className="hr-login-form-wrap">
              ...
              <div className="hr-login-form-header">
                ...
                <p>{t("Sign in to access the HR Portal.")}</p>
              </div>
              ...
              >
                ...
                >
                  {isLoading ? <Spinner size="sm" animation="border" /> : t("Sign in to HR Portal")}
                </motion.button>

                <div className="hr-login-help">
                  {t("Trouble signing in? Contact your HR administrator.")}
                </div>
              </Form>
            </div>

            <div className="hr-login-footer">
              {t("© {{year}} Route Facile · HR Portal", { year: new Date().getFullYear() })}
            </div>
          </div>
        </motion.div>
      </div>
