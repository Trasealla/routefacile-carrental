...
const ChangePassword = () => {
  ...
  const getUserRole = () => {
    const encrypted = localStorage.getItem("routefacile_user_role");
    if (!encrypted) return "";
    try {
  ...
  const handleSubmit = async (event) => {
    ...
    if (newPassword !== confirmPassword) {
      notifyError(t("New password and confirm password do not match."));
      return;
    }

    if (newPassword.length < 8) {
      notifyError(t("New password must be at least 8 characters."));
      return;
    }
    ...
    try {
      ...
      if (!res?.error) {
        ...
        notifyError(res.message || t("Failed to change password."));
      }
    } catch (err) {
      console.error(err);
    ...
    >
      ...
      >
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <h4 style={{ fontWeight: 700, color: "#1e293b" }}>{t("Change Password")}</h4>
        ...
        <Form noValidate validated={validated} onSubmit={handleSubmit}>
          ...
          >
            {isLoading ? (
              ...
              <>
                <FaLock size={14} /> {t("Change Password")}
              </>
            )}
          </button>
        </Form>
