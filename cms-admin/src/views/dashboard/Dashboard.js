...
const Dashboard = () => {
  ...
  const getDahsboardDataFunction = async (from, to, useDummyData = false) => {
    ...
    try {
      ...
      notifyError(t("Failed to load real data - displaying demo data"));
    } finally {
      setLoading(false);
      setOverAllDashApisLoading(false);
    }
  ...
  const fetchSummaryData = async () => {
    ...
    if (!res || !res.data || res.data.length === 0) {
      notifyError(t("No data found for the selected date range"));
      return null;
    }
    return res;
  };
  ...
  const handleExportExcel = async () => {
    try {
      ...
      notifySuccess(t("Excel report downloaded successfully"));
    } catch (err) {
      console.error("Excel export error:", err);
      notifyError(t("Failed to export Excel report"));
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleExportPDF = async () => {
    try {
      ...
      notifySuccess(t("PDF report downloaded successfully"));
    } catch (err) {
      console.error("PDF export error:", err);
      notifyError(t("Failed to export PDF report"));
    } finally {
      setExporting(false);
    }
  };
